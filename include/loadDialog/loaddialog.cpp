...
LoadDialog::LoadDialog(const QString &filePath, QWidget *parent)
    : QDialog(parent) {
  ...
  if (file.open(QIODevice::ReadOnly)) {
    ...
    QMessageBox::information(this, tr("Marea"),
...
void LoadDialog::setProjectMetaData(const ProjectMetaData &metadata) {
  ...
  m_metaDataWidget->setCeroPuestoValueAndUnit(metadata.ceroUnit(),
                                              metadata.ceroPuesto());
  m_metaDataWidget->setNivelReferenciaValueAndUnit(metadata.referenciaUnit(),
                                                   metadata.nivelReferencia());
...
void LoadDialog::getDataPoints() {
  ...
  if (from <= to) {
    ...
    for (int i = 0; i < pointsOfData.size(); ++i) {
      if (dateField <= pointsOfData[i].size() &&
          timeField <= pointsOfData[i].size() &&
          heightField <=
              pointsOfData[i].size()) { // Chequea que el campo exista

        QDate date =
            QDate::fromString(pointsOfData[i].at(dateField - 1), m_dateFormat);
        QTime time =
            QTime::fromString(pointsOfData[i].at(timeField - 1), m_timeFormat);
        ...
        if (date.isValid() && time.isValid() && d_ok) {
          ...
          desition = QMessageBox::question(
              this, tr("Advertencia"),
              tr("Falla al cargar datos. \n"
                 " Posibles Causas: \n"
...
void LoadDialog::enableFieldEditorsEdition() {
  if (m_firstLineLineEdit->text().isEmpty() ||
      m_lastLineLineEdit->text().isEmpty()) {
...
void LoadDialog::enableImportButton() {
  if (m_timeFormat != "otro" && m_dateFormat != "otro" &&
      !m_dateFormat.isEmpty() && !m_timeFormat.isEmpty() &&
      m_fieldDateEdit->value() && m_fieldTimeEdit->value() &&
      m_fieldHeightEdit->value() &&
      m_fieldDateEdit->value() != m_fieldTimeEdit->value() &&
      m_fieldDateEdit->value() != m_fieldHeightEdit->value() &&
      m_fieldTimeEdit->value() != m_fieldHeightEdit->value()) {
...
void LoadDialog::fillFirstAndLastDataPoints() // conectado los edit de primera y
                                              // ultima linea
{
...
void LoadDialog::fillFirstAndLastDate(int index) {
  ...
  if (index <= dataPoint1.size())
    ...
    m_firstPointDateEdit->setText(tr(""));
...
void LoadDialog::fillFirstAndLastTime(int index) {
  ...
  if (index <= dataPoint1.size())
    ...
    m_firstPointTimeEdit->setText(tr(""));
...
void LoadDialog::fillFirstAndLastHeigth(int index) {
  ...
  if (index <= dataPoint1.size())
    ...
    m_firstPointHeigthEdit->setText(tr(""));
...
void LoadDialog::setMinimumLastLineValue(int value) {
  if (m_lastLineLineEdit->value() < value)
    m_lastLineLineEdit->setValue(value);
...
void LoadDialog::settingUpEveryThing() {
  ...
  // m_fieldDateEdit->setDisabled(true);
  connect(m_fieldDateEdit, SIGNAL(valueChanged(int)), this,
          SLOT(enableImportButton()));
  connect(m_fieldDateEdit, SIGNAL(valueChanged(int)), this,
          SLOT(fillFirstAndLastDate(int)));
  m_fieldTimeEdit = new QSpinBox(this);
  // m_fieldTimeEdit->setDisabled(true);
  connect(m_fieldTimeEdit, SIGNAL(valueChanged(int)), this,
          SLOT(enableImportButton()));
  connect(m_fieldTimeEdit, SIGNAL(valueChanged(int)), this,
          SLOT(fillFirstAndLastTime(int)));
  m_fieldHeightEdit = new QSpinBox(this);
  // m_fieldHeightEdit->setDisabled(true);
  connect(m_fieldHeightEdit, SIGNAL(valueChanged(int)), this,
          SLOT(enableImportButton()));
  connect(m_fieldHeightEdit, SIGNAL(valueChanged(int)), this,
          SLOT(fillFirstAndLastHeigth(int)));
  ...
  connect(m_separatorDateEdit, SIGNAL(textChanged(QString)), this,
          SLOT(setTopFieldValidValue()));
  m_separatorTimeEdit = new QLineEdit(this);
  m_separatorTimeEdit->setAlignment(Qt::AlignRight);
  connect(m_separatorTimeEdit, SIGNAL(textChanged(QString)), this,
          SLOT(setTopFieldValidValue()));
  m_separatorHeigthEdit = new QLineEdit(this);
  m_separatorHeigthEdit->setAlignment(Qt::AlignRight);
  connect(m_separatorHeigthEdit, SIGNAL(textChanged(QString)), this,
          SLOT(setTopFieldValidValue()));
  ...
  m_replaceDataAction =
      new QAction(QIcon(":images/replaceData.png"), tr("Remplazar Datos"));
  m_replaceDataAction->setToolTip(tr("Remplazar datos"));
  connect(m_replaceDataAction, SIGNAL(triggered(bool)), this,
          SLOT(getDataPoints()));

  m_appendDataAction =
      new QAction(QIcon(":images/appendData.png"), tr("Agregar Datos"));
  m_appendDataAction->setToolTip(tr("Agregar datos"));
  connect(m_appendDataAction, SIGNAL(triggered(bool)), this,
          SLOT(getDataPointAndAppendData()));
  ...
  m_importButton =
      new QPushButton(QIcon(":images/importButton.png"), tr("Importar"), this);
  m_importButton->setDisabled(true);
  ...
  // m_unitGroupBox = new QGroupBox(tr("Unidad de Medida:")); //GroupBox de
  // Medidas m_correctionGroupBox = new QGroupBox(tr("Corrección de Nivel"));
  // //GrupoBox de Correciones
  m_firstLineGroupBox =
      new QGroupBox(tr("Primera Línea:")); // GroupBox de carga de datos
  m_lastLineGroupBox =
      new QGroupBox(tr("Última Línea:")); // GroupBox de carga de datos
  ...
  connect(m_firstLineLineEdit, SIGNAL(valueChanged(int)), this,
          SLOT(setTopFieldValidValue(int)));
  connect(m_firstLineLineEdit, SIGNAL(valueChanged(int)), this,
          SLOT(setMinimumLastLineValue(int)));
  // connect(m_firstLineLineEdit,SIGNAL(valueChanged(int)),this,SLOT(enableFieldEditorsEdition()));
  connect(m_firstLineLineEdit, SIGNAL(valueChanged(int)), this,
          SLOT(fillFirstAndLastDataPoints()));
  ...
  connect(m_lastLineLineEdit, SIGNAL(valueChanged(int)), this,
          SLOT(fillFirstAndLastDataPoints()));
  ...
  m_dateFormatLineEdit->setDisabled(true);
  connect(m_dateFormatLineEdit, SIGNAL(textEdited(QString)), this,
          SLOT(setCustomDateFormat(QString)));
  connect(m_dateFormatLineEdit, SIGNAL(textEdited(QString)), this,
          SLOT(enableImportButton()));
  ...
  connect(m_dateFormatComboBox, SIGNAL(currentIndexChanged(QString)), this,
          SLOT(setDateFormat(QString)));
  ...
  m_timeFormatLineEdit->setDisabled(true);
  connect(m_timeFormatLineEdit, SIGNAL(textEdited(QString)), this,
          SLOT(setCustomTimeFormat(QString)));
  connect(m_timeFormatLineEdit, SIGNAL(textChanged(QString)), this,
          SLOT(enableImportButton()));
  ...
  connect(m_timeFormatComboBox, SIGNAL(currentIndexChanged(QString)), this,
          SLOT(setTimeFormat(QString)));
