...
MyCoordinatesEditorWidget::MyCoordinatesEditorWidget(QWidget *parent)
  ...
  // m_coordinatesSpinBox->setSizePolicy(QSizePolicy::Ignored,
  ...
  connect(m_coordinatesSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setCoordinate(double)));
  connect(this, SIGNAL(hTypeChanged(HemisphereType)), this,
          SLOT(setHemisphereComboBoxItems(HemisphereType)));
}
...
double MyCoordinatesEditorWidget::coordinate() const { return m_coordinate; }

MyCoordinatesEditorWidget::HemisphereType
MyCoordinatesEditorWidget::hType() const {
  return m_hType;
}
...
void MyCoordinatesEditorWidget::setHType(
    MyCoordinatesEditorWidget::HemisphereType hType) {
  if (m_hType == hType)
    return;
...
void MyCoordinatesEditorWidget::setHemisphereComboBoxItems(
    MyCoordinatesEditorWidget::HemisphereType hType) {
  if (hType == latitud) {
