...
HoraCotidianaSemidiurna::HoraCotidianaSemidiurna(const HarmonicConstant &M2,
...
double HoraCotidianaSemidiurna::shallowWaterInfluence1() {
  double a = 2 * m_M4.amplitud() *
             qSin(qDegreesToRadians(2 * m_M2.phase() - m_M4.phase()));

  double b = 3 * m_M6.amplitud() *
             qSin(qDegreesToRadians(3 * m_M2.phase() - m_M6.phase()));

  double c = 4 * m_M4.amplitud() *
             qCos(qDegreesToRadians(2 * m_M2.phase() - m_M4.phase()));

  double d = m_M2.amplitud() +
             9 * m_M6.amplitud() *
