...
#include "Model_View/hctabledelegate.h"
#include <QComboBox>
#include <QDateEdit>
#include <QGroupBox>
#include <QHBoxLayout>
...
#include "Model_View/harmonicconstantfrequencytabledelegate.h"
#include "Model_View/harmonicconstantfulltablemodel.h"
...
SchemeWidget::SchemeWidget(
    ...
    : QDialog(parent), m_schemesLabels(schemesLabels),
      m_componentsLabels(componentsLabels),
      m_schemes_componentMap(schemes_componentMap),
      m_schemeDescriptionMap(schemeDescription) {
...
void SchemeWidget::setHarmonicConstantModelData(
...
void SchemeWidget::setSchemesComponentsMap(
    const QMap<QString, QMap<QString, bool>> &schemes_componentMap) {
  m_schemes_componentMap = schemes_componentMap;
}

void SchemeWidget::createConfigSchemeDialog() {
  m_configSchemeDialog = new SchemeConfigDialog(
      m_schemesLabels, m_componentsLabels, m_schemes_componentMap,
      m_schemeDescriptionMap, this, Qt::WindowCloseButtonHint);
  connect(m_configSchemeDialog, SIGNAL(acceptButtonClicked()), this,
...
void SchemeWidget::selectAnalisisScheme(const QString &str) {
  ...
  // new code
  m_schemeDescriptionPlainTextEdit->setPlainText(
      m_schemeDescriptionMap.value(str));

  if (str == "-Selecciona un esquema de análisis-")
    m_analizarPushButton->setEnabled(false);
  else
    m_analizarPushButton->setEnabled(true);
}
...
void SchemeWidget::changeScheme(const QString &newScheme) {
  m_tidalSchemeComponents->setCheckBoxesStatus(
      m_schemes_componentMap.value(newScheme));
}
...
void SchemeWidget::crearComponentes(const QStringList &schemesLabels,
                                    const QStringList &componentsLabels) {
  /*m_loadingQuickWidget = new
  QQuickWidget(QUrl(QStringLiteral("qrc:/analizing.qml")));
  ...
  m_schemeComboBox->addItem(tr("-Selecciona un esquema de análisis-"));
  m_schemeComboBox->addItems(schemesLabels);
  connect(m_schemeComboBox, SIGNAL(currentIndexChanged(QString)), this,
          SLOT(selectAnalisisScheme(QString)));
  ...
  connect(m_configEsquemaToolButton, SIGNAL(clicked(bool)), this,
          SLOT(createConfigSchemeDialog()));

  m_analizarPushButton =
      new QPushButton(QIcon(":images/analisis.png"), tr("Analizar"), this);
  ...
  m_analizarPushButton->setEnabled(false);
  connect(m_analizarPushButton, SIGNAL(clicked(bool)), this,
          SLOT(beginHarmonicAnalisis()));

  m_tableFrame = new QFrame;
  m_tableFrame->hide();

  m_saveHarmonicConstantsButton =
      new QPushButton(QIcon(":images/writeToFile1.png"), tr("Guardar"));
  m_saveHarmonicConstantsButton->setToolTip(
      tr("Guardar Constantes Armónicas a archivo ASCII"));
  m_saveHarmonicConstantsButton->setEnabled(false);
  connect(m_saveHarmonicConstantsButton, SIGNAL(clicked(bool)), this,
          SIGNAL(saveHarmonicConstantsButtonClicked()));

  m_uploadHCToDataBase =
      new QPushButton(QIcon(":images/data-upload.png"), tr("Registrar"));
  m_uploadHCToDataBase->setToolTip(
      tr("Agregar set de constantes armónicas al registro"));
  m_uploadHCToDataBase->setEnabled(false);
  connect(m_uploadHCToDataBase, SIGNAL(clicked(bool)), this,
          SIGNAL(uploadHCButtonClicked()));
  ...
  int width = 15;
  for (int i = 0;
       i < m_harmonicConstantTableView->model()->columnCount(QModelIndex());
       ++i) {
  ...
  m_saveSelectedData = new QPushButton(QIcon(":images/save.png"),tr("Guardar
  Datos"));
  connect(m_saveSelectedData,SIGNAL(clicked(bool)),this,SIGNAL(saveDataButtonClicked()));*/

  //********************************************************************
  m_animation =
      new QPropertyAnimation(m_harmonicConstantTableView, "geometry", this);
...
QQuickWidget(QUrl(QStringLiteral("qrc:/analizing.qml")));
    ...
    QSurfaceFormat format;
    if (QCoreApplication::arguments().contains(QStringLiteral("--coreprofile")))
{ format.setVersion(4, 4); format.setProfile(QSurfaceFormat::CoreProfile);
    }
    if (QCoreApplication::arguments().contains(QStringLiteral("--multisample")))
        format.setSamples(4);
    m_loadingQuickWidget->setFormat(format);
