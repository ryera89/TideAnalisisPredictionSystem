...
SamplingDialog::SamplingDialog(
    const QVector<TidesMeasurement> &inputMeasurements, QWidget *parent)
  ...
  m_mapForEndDatePos[inputMeasurements.last().measurementDate()] =
      inputMeasurements.size() - 1;
  int i = 0;
  ...
  m_infoLabel = new QLabel(tr(
      "Sampling: Reduce el número total de datos de entrada mediante\n"
      "la eliminación de datos sujetos a un intervalo de tiempo específico.\n"
      "Especificar un intervalo de tiempo muy largo puede causar la "
      "eliminación\n"
  ...
  m_fechaInicialEdit =
      new QDateEdit(inputMeasurements.first().measurementDate());
  m_fechaInicialEdit->setDateRange(inputMeasurements.first().measurementDate(),
                                   inputMeasurements.last().measurementDate());
  m_fechaInicialEdit->setCalendarPopup(true);
  m_fechaInicialEdit->setFixedWidth(100);
  connect(m_fechaInicialEdit, SIGNAL(dateChanged(QDate)), this,
          SLOT(updateFinalDateEditRange(QDate)));

  m_fechaFinalEdit = new QDateEdit(inputMeasurements.last().measurementDate());
  m_fechaFinalEdit->setDateRange(inputMeasurements.first().measurementDate(),
                                 inputMeasurements.last().measurementDate());
  m_fechaFinalEdit->setCalendarPopup(true);
  m_fechaFinalEdit->setFixedWidth(100);
  connect(m_fechaFinalEdit, SIGNAL(dateChanged(QDate)), this,
          SLOT(updatePotentialPoints()));
  ...
  connect(m_intervaloEdit, SIGNAL(timeChanged(QTime)), this,
          SLOT(updatePotentialPoints()));
...
void SamplingDialog::sampleData() {
  ...
  quint64 intervalInSecs = m_intervaloEdit->time().hour() * 3600 +
                           m_intervaloEdit->time().minute() * 60;
  ...
  /*bool flag = true; //flag para igualar la fecha auxiliar a la primera para
  agregar foreach (TidesMeasurement measurement, m_allData) { if
  (measurement.measurementDate() >= iniDate && measurement.measurementDate() <=
  endDate){ if (flag){ auxDateTime = measurement.measurementDateTime(); flag =
  false;
         }
...
void SamplingDialog::updateFinalDateEditRange(const QDate &iniDate) {
  ...
  if (iniDate <= m_fechaFinalEdit->date())
...
void SamplingDialog::updatePotentialPoints() {
  ...
  QDateTime beginDateTime =
      m_allData.at(m_mapForFirstDatePos.value(inDate)).measurementDateTime();
  QDateTime endDateTime =
      m_allData.at(m_mapForEndDatePos.value(endDate)).measurementDateTime();
...
void SamplingDialog::updateStatusText(quint64 datosEntrada,
                                      quint64 datosSalidaPotenciales,
                                      quint64 currentAddedPoints) {
  m_statusLabel->setText(
      tr("Datos de Entrada: <font color = red> %1 </font>. ")
          .arg(datosEntrada) +
      tr("Datos de Salida [potenciales] <font color = blue> %1 </font>. ")
          .arg(datosSalidaPotenciales) +
      tr("Datos de Salida [reales] <font color = green> %1 </font>")
          .arg(currentAddedPoints));
}
