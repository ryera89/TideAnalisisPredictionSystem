...
NivelacionAcuaticaTableModel::NivelacionAcuaticaTableModel(QObject *parent)
...
QVariant NivelacionAcuaticaTableModel::data(const QModelIndex &index,
                                            int role) const {
  if (!index.isValid())
    return QVariant();
    ...
    if (role == Qt::DisplayRole) {
      ...
      case 4: {
        if (m_metodo == UnPuestoPermanente)
          ...
          return m_puestoPermanente2DataSeries.at(row).seaLevel();
        break;
      }
      case 5:
        return m_diffConPuestoPermanente1DataSeries.at(row).seaLevel();
...
QVariant NivelacionAcuaticaTableModel::headerData(int section,
...
void NivelacionAcuaticaTableModel::setPuestoProvisionalDataSet(
    const QVector<TidesMeasurement> &puestoProvicionalData) {
  beginResetModel();
  ...
  m_diffConPuestoPermanente1DataSeries.resize(
      m_puestoProvisionalDataSeries.size());
  m_diffConPuestoPermanente2DataSeries.resize(
      m_puestoProvisionalDataSeries.size());
  for (int i = 0; i < m_puestoProvisionalDataSeries.size(); ++i) {
    m_diffConPuestoPermanente1DataSeries[i] =
        m_puestoProvisionalDataSeries.at(i) -
        m_puestoPermanente1DataSeries.at(i);
    m_diffConPuestoPermanente2DataSeries[i] =
        m_puestoProvisionalDataSeries.at(i) -
        m_puestoPermanente2DataSeries.at(i);
  }
  endResetModel();
}

void NivelacionAcuaticaTableModel::setPuestoPermanente1DataSet(
    const QVector<TidesMeasurement> &puestoPermanente1Data) {
  beginResetModel();
...
void NivelacionAcuaticaTableModel::setPuestoPermanente2DataSet(
    const QVector<TidesMeasurement> &puestoPermanente2Data) {
  beginResetModel();
...
void NivelacionAcuaticaTableModel::setMetodoDeNivelacion(
    NivelacionAcuaticaTableModel::MetodoDeNivelacion metodo) {
  if (m_metodo != metodo) {
    m_metodo = metodo;
