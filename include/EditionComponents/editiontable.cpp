...
EditionTable::EditionTable(QWidget *parent):QTableView(parent){
    ...
    connect(verHeader,SIGNAL(removeRowActionTriggered(bool)),this,SLOT(removeRows()));
    connect(verHeader,SIGNAL(insertRowActionTriggered(bool)),this,SLOT(insertRow()));
...
void EditionTable::keyPressEvent(QKeyEvent *event)
{
        ...
        case Qt::Key_Return:{
            ...
            if (index.isValid() && (index.row()+1) < this->model()->rowCount()){
                QModelIndex nextIndex = this->model()->index(index.row()+1, index.column());
