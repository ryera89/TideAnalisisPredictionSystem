...
ManualDataIntroductionWidget::ManualDataIntroductionWidget(QWidget *parent)
    : QDialog(parent) {
...
void ManualDataIntroductionWidget::setProjectMetaData(
...
void ManualDataIntroductionWidget::updateTimeInterval(int index) {
  ...
  case 2:
    ...
    break;
  case 3:
    m_timeInterval = m_timeIntervalTimeEdit->time().hour() * 3600 +
                     m_timeIntervalTimeEdit->time().minute() * 60 +
                     m_timeIntervalTimeEdit->time().second();
    break;
  default:
    break;
  }
...
void ManualDataIntroductionWidget::clearMeasurements() {
  if (m_model->rowCount(QModelIndex()) > 0) {
    int button = QMessageBox::question(this, tr("Introducción manual de datos"),
                                       tr("Se borraran todos los datos,"
                                          "¿Desea continuar?"));
    if (button == QMessageBox::Yes) {

      m_model->removeRows(0, m_model->rowCount(QModelIndex()) - 1,
                          QModelIndex());
    }
  }
}
...
void ManualDataIntroductionWidget::generateData() {
  QDateTime begin =
      QDateTime(m_iniDateEdit->date(), m_iniTimeEdit->time(), Qt::UTC);
  QDateTime end =
      QDateTime(m_endDateEdit->date(), m_endTimeEdit->time(), Qt::UTC);

  if (begin > end) {
    QMessageBox::information(
        this, tr("Error de Tiempo"),
        tr("El momento de tiempo de inicio de las mediciones /n "
  ...
  if (seconds % m_timeInterval) {
    QMessageBox::information(
        this, tr("Error de Tiempo"),
        tr("El intervalo de medición no coincide con el tiempo final"));
    return;
  }
...
void ManualDataIntroductionWidget::createComponents() {
  ...
  connect(m_editionTable, SIGNAL(rowsRemoved(int, int)), this,
          SLOT(removeRows(int, int)));
  connect(m_editionTable, SIGNAL(rowInserted()), this,
          SLOT(insertMeasurement()));
  ...
  connect(m_measurementUnitComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setConversionUnit(int)));
  ...
  connect(m_iniDateEdit, SIGNAL(dateChanged(QDate)), this,
          SLOT(setMinimumDateEditValue(QDate)));
  ...
  connect(m_timeIntervalComboBox, SIGNAL(activated(int)), this,
          SLOT(enableTimeIntervalEdit(int)));
  connect(m_timeIntervalComboBox, SIGNAL(activated(int)), this,
          SLOT(updateTimeInterval(int)));
  ...
  m_timeIntervalTimeEdit->setDisabled(true);
  connect(m_timeIntervalTimeEdit, SIGNAL(timeChanged(QTime)), this,
          SLOT(setTimeInterval(QTime)));
  ...
  ini_endGridLayout->addWidget(m_timeIntervalComboBox, 1, 2, 1, 1,
                               Qt::AlignTop);
  ...
  ini_endGridLayout->addWidget(m_timeIntervalTimeEdit, 4, 2, 1, 1,
                               Qt::AlignTop);
  ini_endGridLayout->addWidget(m_genPushButton, 5, 2, 1, 1);

  m_ini_endGroupBox->setLayout(ini_endGridLayout);

  // m_timeIntervalGroupBox = new QGroupBox(tr("Intervalo de
  // Medición"),m_mainGroupBox);
  ...
  m_insertPushButton =
      new QPushButton(QIcon(":images/table_row_new.png"), tr("Insertar"));
  m_insertPushButton->setToolTip(tr("Insertar fila"));
  connect(m_insertPushButton, SIGNAL(clicked(bool)), this,
          SLOT(insertMeasurement()));

  m_removePushButton =
      new QPushButton(QIcon(":images/table_row_delete.png"), tr("Eliminar"));
  m_removePushButton->setToolTip(tr("Eliminar fila"));
  connect(m_removePushButton, SIGNAL(clicked(bool)), m_editionTable,
          SLOT(removeRows()));

  m_clearPushButton =
      new QPushButton(QIcon(":images/table_delete.png"), tr("Limpiar"));
  m_clearPushButton->setToolTip(tr("Limpiar tabla"));
  connect(m_clearPushButton, SIGNAL(clicked(bool)), this,
  ...
  m_okPushButton =
      new QPushButton(QIcon(":images/importButton.png"), tr("Importar"));
...
double
ManualDataIntroductionWidget::determineLevelFromCorr(const double &corr) {
