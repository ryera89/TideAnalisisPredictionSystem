...
void MyHeader::createContextMenu() {
  ...
  connect(m_removeRowAction, SIGNAL(triggered(bool)), this,
          SIGNAL(removeRowActionTriggered(bool)));

  m_contextMenu->addAction(m_insertRowAction);
  m_contextMenu->addAction(m_removeRowAction);
  connect(m_insertRowAction, SIGNAL(triggered(bool)), this,
          SIGNAL(insertRowActionTriggered(bool)));
}
