...
HarmonicConstant::HarmonicConstant():m_name(QString()),m_frequency(0.0),m_C(0.0),m_S(0.0),m_amplitud(0.0),m_uncorrectedPhase(0.0),m_phase(0.0),m_correctedPhase(0.0),m_origin(SOLAR){}
...
HarmonicConstant::HarmonicConstant(const QString &name, const double &frequency, const DoodsonNumbers &doodsonNumber,Origin orig):m_name(name),m_frequency(frequency)
  , m_doodsonNumber(doodsonNumber), m_origin(orig), m_C(0.0),m_S(0.0),m_amplitud(0.0),m_uncorrectedPhase(0.0), m_phase(0.0),m_correctedPhase(0.0) {}
...

{
    ...
    if (C != 0.0){
        m_uncorrectedPhase = qRadiansToDegrees((qAtan(S/C)));

        //NOTE: Ver como afecta esto al resultado de las fases
        if (S > 0.0 && C < 0.0) m_uncorrectedPhase+=180;
        if (S < 0.0 && C < 0.0) m_uncorrectedPhase+=180;
        if (S < 0.0 && C > 0.0) m_uncorrectedPhase+=360;
    }else m_uncorrectedPhase = 90.0;
