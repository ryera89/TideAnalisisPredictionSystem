...
AstronomicalMeanLongitudes::AstronomicalMeanLongitudes(
...
void AstronomicalMeanLongitudes::determineLongitudesByCartwright() {
  ...
  double fraction =
      (m_datetime.time().hour() + m_datetime.time().minute() / 60.0) / 24.0 -
      0.5;
...
void AstronomicalMeanLongitudes::determineLongitudesBySchureman() {
  ...
  m_s = 270.0 + 26.0 / 60.0 + 14.72 / 3600.0 +
        (1336.0 * 360.0 + 1108411.20 / 3600.0) * T + 9.09 / 3600.0 * T_square +
        0.0068 / 3600.0 * T_cubic;

  m_h = 279.0 + 41.0 / 60.0 + 48.04 / 3600.0 + 129602768.13 / 3600.0 * T +
        1.089 / 3600.0 * T_square;

  m_p = 334.0 + 19.0 / 60.0 + 40.87 / 3600.0 +
        (11.0 * 360.0 + 392515.94 / 3600.0) * T - 37.24 / 3600.0 * T_square -
        0.045 / 3600.0 * T_cubic;

  m_N = 259.0 + 10.0 / 60.0 + 57.12 / 3600.0 -
        (5.0 * 360.0 + 482912.63 / 3600.0) * T + 7.58 / 3600.0 * T_square +
        0.008 / 3600.0 * T_cubic;

  m_Ps = 281.0 + 13.0 / 60.0 + 15.0 / 3600.0 + 6189.03 / 3600.0 * T +
         1.63 / 3600.0 * T_square + 0.012 / 3600.0 * T_cubic;
...
void AstronomicalMeanLongitudes::printAstronomicalLongitudesToConsole() {
  ...
  std::cout << "Date: "
            << m_datetime.toString("dd/MM/yyyy hh:mm:ss").toStdString()
            << std::endl;
...
void AstronomicalMeanLongitudes::determineValues_I() {
  m_I = qRadiansToDegrees(
      qAcos(0.91370 - 0.03569 * qCos(qDegreesToRadians(m_N))));
...
void AstronomicalMeanLongitudes::determineValues_vprima() {
  double num =
      qSin(qDegreesToRadians(m_v)) * qSin(qDegreesToRadians(2.0 * m_I));
  double den =
      qCos(qDegreesToRadians(m_v)) * qSin(qDegreesToRadians(2.0 * m_I)) +
      0.3347;
...
void AstronomicalMeanLongitudes::determineValues_2v2prima() {
  double num = qSin(qDegreesToRadians(2 * m_v)) *
               qPow(qSin(qDegreesToRadians(m_I)), 2.0);
  double den = qCos(qDegreesToRadians(2 * m_v)) *
                   qPow(qSin(qDegreesToRadians(m_I)), 2.0) +
               0.0727;
...
void AstronomicalMeanLongitudes::determineRa() {
  double P = m_p - m_ep;
  double term1 = 12.0 * qPow(qTan(qDegreesToRadians(m_I / 2.0)), 2.0) *
                 qCos(qDegreesToRadians(2.0 * P));
...
void AstronomicalMeanLongitudes::determineQa() {
  ...
  double term3 = (3.0 / 2.0) *
                 (1 - qPow(qTan(qDegreesToRadians(m_I / 2.0)), 2.0)) *
                 qCos(qDegreesToRadians(2.0 * (m_p - m_ep)));
  ...
  // double term1
  // = 3.0*qCos(qDegreesToRadians(m_I))/qPow(qCos(qDegreesToRadians(m_I/2.0)),2);
