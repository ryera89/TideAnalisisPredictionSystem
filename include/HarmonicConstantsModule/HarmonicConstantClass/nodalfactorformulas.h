...
// Formula para el calculo del factor nodal de la componente lunar de largo
// periodo Mf f = pow(sin(I),2)/0.1578
double NodalAmplitudeFactor_Mf(const double &I);
// u = -2ξ
double NodalAngleFactor_Mf(const double &e);

// Formula para el calculo del factor nodal de las componentes lunares diurnas
// O1,Q1,2Q1,RHO1,SIGMA1,SO3 f = sin(I)*pow(cos(I/2),2)/0.3800
...
double NodalAngleFactor_O1_Q1_2Q1_RHO1_SIGMA1_SO3(const double &e,
                                                  const double &v);
// Formula para el calculo del factor nodal de las componentes lunares diurnas
// J1,CHI1,THETA1,MP1,SO1 f = sin(2I)/0.7214
...
// Formula para el calculo del factor nodal de las componentes lunares diurnas
// OO1, KQ1 f = sin(I)*pow(sin(I/2),2)/0.0164
...
// Formula para el calculo del factor nodal de las componentes lunares
// semidiurnas M2,N2,2N2,NU2,LAMDA2,MU2,2SM2,MS4,2SM6 f = pow(cos(I/2),4)/0.9154
double
...
// Formula para el calculo del factor nodal de la componente lunar semidiurna
// KJ2 f = pow(sin(I),2)/0.1565
...
// Formula para el calculo del factor nodal de las componentes
...
// LUNAR_DIURNAL_1 componentes lunares diurnas O1,Q1,2Q1,RHO1,SIGMA1,SO3 u =
// 2ξ-ν, f = sin(I)*pow(cos(I/2),2)/0.3800 LUNAR_DIURNAL_2 componentes lunares
// diurnas J1,CHI1,THETA1,MP1,SO1 u = -ν, f = sin(2I)/0.7214 LUNAR_DIURNAL_3
// lunares diurnas OO1, KQ1 f = sin(I)*pow(sin(I/2),2)/0.0164, u = -2ξ-ν
// COMBINATION_DIURNAL_1 componente M1 f = sin(I)*pow(cos(I/2),2)/(0.3800*Qa) =
// f(O1)/Qa, u = ξ-ν+Q COMBINATION_DIURNAL_2 componentes K1, SK3, f =
// sqrt(0.8965*pow(sin(2I),2)+0.6001*sin(2I)cos(ν) + 0.1006),u = - ν’
// LUNAR_SEMIDIURNAL_1 componentes lunares semidiurnas
// M2,N2,2N2,NU2,LAMDA2,MU2,2SM2,MS4,2SM6 f = pow(cos(I/2),4)/0.9154, u = 2ξ-2ν
// LUNAR_SEMIDIURNAL_1_2 componente 2SM2 f = f = pow(cos(I/2),4)/0.9154, u =
// -2ξ+2ν LUNAR_SEMIDIURNAL_2 componente lunar semidiurna KJ2 f =
// pow(sin(I),2)/0.1565, u = -2ν COMBINATION_SEMIDIURNAL_1 componente semidiurna
// L2 f(M2)/Ra, u = 2ξ-2ν-R COMBINATION_SEMIDIURNAL_2 componente semidiurna K2 f
// = sqrt(19.0444*pow(sin(I)),4)+2.7702*pow(sin(I),2)*cos(2ν) + 0.0981), u =
// -2ν'' LUNAR_TERDIURNAL componente M3 f = pow(cos(I/2),6)/0.8758, u = 3ξ-3ν
// SHALLOW_1 las componentes MNS2,M4,MN4,2MS6,MSN6,2(MS)8 f = pow(f(M2),2), u =
// 4ξ-4ν = 2*u(M2) SHALLOW_2 componentes M6,2MN6,3MS8,2MSN8 f = pow(f(M2),3), u
// = 6ξ-6ν = 3*u(M2) SHALLOW_3 componentes M8 f = pow(f(M2),4), u = 8ξ-8ν =
// 4*u(M2) SHALLOW_COMB_1 componentes MK3 f = f(M2)*f(K1), u = 2ξ-2ν-ν’
