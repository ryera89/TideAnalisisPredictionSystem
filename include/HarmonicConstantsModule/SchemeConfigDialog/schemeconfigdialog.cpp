...
SchemeConfigDialog::SchemeConfigDialog(const QStringList &schemesLabels, const QStringList &componentsLabels, const QMap<QString,QMap<QString,bool>> &schemes_componentMap,const QMap<QString,QString> &schemeDescription, QWidget *parent, Qt::WindowFlags f):
    QDialog(parent,f),m_componentsList(componentsLabels),m_schemes_componentMap(schemes_componentMap),m_schemesDescriptionMap(schemeDescription)
{
...
QMap<QString, QMap<QString, bool> > SchemeConfigDialog::schemeComponentStatus() const
{
...
void SchemeConfigDialog::updateSchemeDescription()
{
    ...
    if (index.isValid()){
        QString schemeName = m_schemesList->model()->data(index,Qt::DisplayRole).toString();
...
void SchemeConfigDialog::updateSchemeComponentStatus(const QString &label, bool status)
{
   QModelIndex index = m_schemesList->currentIndex();
...
void SchemeConfigDialog::handleSchemeRename(const QString &oldName, const QString &newName)
{
    if (oldName != newName){
    m_schemes_componentMap.insert(newName,m_schemes_componentMap.value(oldName));
    m_schemes_componentMap.remove(oldName);

    //new code
    m_schemesDescriptionMap.insert(newName,m_schemesDescriptionMap.value(oldName));
...
bool SchemeConfigDialog::writeSchemesFile()
{
    ...
    if (file.open(QIODevice::WriteOnly)){
        ...
        out << (uint)m_myModel->stringList().size() << (uint)m_componentsList.size();
        ...
        foreach (QString schemeName, m_myModel->stringList()) {
            ...
            foreach (QString componentName, m_componentsList) {
                out << componentName << m_schemes_componentMap.value(schemeName).value(componentName);
            }
        }
...
void SchemeConfigDialog::createComponent(const QStringList &schemesLabels, const QStringList &componentsLabels)
{
    ...
    connect(m_myDelegate,SIGNAL(schemeNameChanged(QString,QString)),this,SLOT(handleSchemeRename(QString,QString)));
    m_schemesList->setItemDelegate(m_myDelegate); //My Own Delegate.
    QModelIndex index = m_schemesList->model()->index(0,0);
    if (index.isValid()) m_schemesList->selectionModel()->select(index,QItemSelectionModel::SelectCurrent);

    connect(m_schemesList,SIGNAL(currentIndexChanged(QModelIndex,QModelIndex)),this,SLOT(selectAnalisisScheme(QModelIndex,QModelIndex)));
    connect(m_schemesList,SIGNAL(schemeDeleted(QString)),this,SLOT(handleSchemeRemoved(QString)));
    ...
    connect(m_tidalSchemeComponents,SIGNAL(checkBoxStatusChanged(QString,bool)),this,SLOT(updateSchemeComponentStatus(QString,bool)));
...
void SchemeConfigDialog::settingLayout()
{
    ...
    QHBoxLayout *descriptionLayout = new QHBoxLayout; //Layout del edit para la descripcion del esquema
