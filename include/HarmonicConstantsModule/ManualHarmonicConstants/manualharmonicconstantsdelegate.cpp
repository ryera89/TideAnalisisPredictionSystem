...
ManualHarmonicConstantsDelegate::ManualHarmonicConstantsDelegate(
    QObject *parent)
    : QStyledItemDelegate(parent) {}

void ManualHarmonicConstantsDelegate::paint(QPainter *painter,
...
QWidget *ManualHarmonicConstantsDelegate::createEditor(
    QWidget *parent, const QStyleOptionViewItem &option,
    const QModelIndex &index) const {
  if (index.column() == 2) {
    QDoubleSpinBox *ampEditor = new QDoubleSpinBox(parent);
    ampEditor->setDecimals(6);
    connect(ampEditor, SIGNAL(editingFinished()), this,
            SLOT(commitAndCloseAmpEditor()));
    return ampEditor;
  }
  if (index.column() == 3) {
    QDoubleSpinBox *phaseEditor = new QDoubleSpinBox(parent);
    phaseEditor->setDecimals(3);
    connect(phaseEditor, SIGNAL(editingFinished()), this,
            SLOT(commitAndClosePhaseEditor()));
    return phaseEditor;
  }
...
void ManualHarmonicConstantsDelegate::setEditorData(
...
void ManualHarmonicConstantsDelegate::setModelData(
