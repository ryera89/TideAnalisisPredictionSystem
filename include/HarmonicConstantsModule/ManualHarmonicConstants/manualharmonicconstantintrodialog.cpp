    ...
    QDialog(parent,Qt::WindowCloseButtonHint)
{
    ...
    connect(m_acceptButton,&QPushButton::clicked,this,&ManualHarmonicConstantIntroDialog::saveToDataBase);

    m_cancelButton = new QPushButton(QIcon(":images/No.png"),tr("Cancelar"));
    connect(m_cancelButton,&QPushButton::clicked,this,&ManualHarmonicConstantIntroDialog::close);
...
void ManualHarmonicConstantIntroDialog::determineCorrectedPhaseForHCVector()
{
   ...
   for (int i = 0; i < m_hcVector.size(); ++i){
       ...
       double cphase = phase - m_hcVector.at(i).frequency()*timeOffset + m_hcVector.at(i).doodsonNumbers().D1()*longitud;

        m_hcVector[i].setCorrectedPhase(cphase);
   }
}

void ManualHarmonicConstantIntroDialog::selectManualIntroducedHarmonicConstant()
{
    m_hcVector.clear();
...
/*QVector<HarmonicConstant> ManualHarmonicConstantIntroDialog::manualIntroducedHarmonicConstant() const
{
...
void ManualHarmonicConstantIntroDialog::uploadHCToDataBase()
{
    if (m_hcVector.isEmpty()){
        QMessageBox::information(this,tr("Error Al Subir Constantes Armónicas"),tr("Todas las constantes armónicas tienen"
                                                                                   " amplitud nula." ));
        ...
        if (m_provincia.isEmpty() || m_localidad.isEmpty()){
            QMessageBox::information(this,tr("Error Al Subir Constantes Armónicas"),tr("Faltan datos necesarios para el correcto guardado de "
                                                                                               "las Constantes Armónicas."));
        }else{
            QString fileName(m_provincia + "_" + m_localidad + "_" + "MANUAL"
                             + "_" + "MANUAL" + ".hcr");
...
bool ManualHarmonicConstantIntroDialog::saveHCToDataBase(const QString &filePath)
{
    QFile file(filePath);
...
bool ManualHarmonicConstantIntroDialog::saveHCInfoToDataBase(const QString &filePath)
{
    QFile file(filePath);
