...
void ReportWizard::setHarmonicConstants(
...
void ReportWizard::accept() {
  ...
  cursor.insertText(m_generalidadesPage->descripcionMareografo(),
                    normalCharFormat);
  cursor.insertBlock();

  cursor.insertBlock(textBlockFormat);
  cursor.insertText(tr("Descripción de la instalación del mareógrafo: "),
                    boldCharFormat);
  cursor.insertText(m_generalidadesPage->descripcionInstalacionMareografo(),
                    normalCharFormat);
  cursor.insertBlock();

  cursor.insertBlock(textBlockFormat);
  cursor.insertText(tr("Datos sobre averías en los dispositivos de nivel: "),
                    boldCharFormat);
  cursor.insertText(m_generalidadesPage->descripcionAverias(),
                    normalCharFormat);
  cursor.insertBlock();
  ...
  cursor.insertText(m_chapasReferenciasPage->descripcionChapaPartida(),
                    normalCharFormat);
  cursor.insertBlock();

  cursor.insertBlock(textBlockFormat);
  cursor.insertText(tr("Datos de la chapa de principal: "), boldCharFormat);
  cursor.insertText(m_chapasReferenciasPage->descripcionChapaPrincipal(),
                    normalCharFormat);
  cursor.insertBlock();

  cursor.insertBlock(textBlockFormat);
  cursor.insertText(tr("Datos de la chapa de trabajo: "), boldCharFormat);
  cursor.insertText(m_chapasReferenciasPage->descripcionChapaTrabajo(),
                    normalCharFormat);
  cursor.insertBlock();
  ...
  cursor.insertText(m_ceroProfundidadPage->ceroDeProfundidad(),
                    normalCharFormat);

  cursor.insertBlock(textBlockFormat);
  cursor.insertText("Método de determinacion: ", boldCharFormat);
  cursor.insertText(m_ceroProfundidadPage->metodoCeroDeProfundidad(),
                    normalCharFormat);

  cursor.insertBlock(textBlockFormat);
  cursor.insertText("Período de observaciones: ", boldCharFormat);
  cursor.insertText(m_ceroProfundidadPage->periodoObsCeroDeProfundidad(),
                    normalCharFormat);
  // cursor.insertBlock();

  cursor.insertBlock(textBlockFormat);
  cursor.insertText(m_ceroProfundidadPage->situacionCeroProfundidad(),
                    boldCharFormat);

  cursor.insertBlock(subTextBlockFormat);
  cursor.insertText(tr("-Cero del puesto: "), boldCharFormat);
  cursor.insertText(m_ceroProfundidadPage->situacionCeroProfCeroPuesto(),
                    normalCharFormat);

  cursor.insertBlock(subTextBlockFormat);
  cursor.insertText(tr("-Nivel medio del mar: "), boldCharFormat);
  cursor.insertText(m_ceroProfundidadPage->situacionCeroProfNMM(),
                    normalCharFormat);

  cursor.insertBlock(subTextBlockFormat);
  cursor.insertText(tr("-Chapa principal: "), boldCharFormat);
  cursor.insertText(m_ceroProfundidadPage->situacionCeroProfChapaPrincipal(),
                    normalCharFormat);

  // cursor.insertBlock();
  cursor.insertBlock(textBlockFormat);
  cursor.insertText(
      m_ceroProfundidadPage->oscilacionesRespectoACeroProfundidad(),
      boldCharFormat);
  ...
  for (int i = 0; i < m_hcVector.size(); ++i) {
    ...
    cursor.setBlockFormat(cellsFormat);
    cursor.insertText(QString::number(m_hcVector.at(i).frequency(), 'f', 6),
                      normalCharFormat);

    cursor = hcTable->cellAt(i + 1, 2).firstCursorPosition();
    cursor.setBlockFormat(cellsFormat);
    cursor.insertText(QString::number(m_hcVector.at(i).amplitud(), 'f', 6),
                      normalCharFormat);

    cursor = hcTable->cellAt(i + 1, 3).firstCursorPosition();
    cursor.setBlockFormat(cellsFormat);
    cursor.insertText(QString::number(m_hcVector.at(i).phase(), 'f', 3),
                      normalCharFormat);

    cursor = hcTable->cellAt(i + 1, 4).firstCursorPosition();
    cursor.setBlockFormat(cellsFormat);
    cursor.insertText(
        QString::number(m_hcVector.at(i).correctedPhase(), 'f', 3),
        normalCharFormat);
  }

  cursor.setPosition(
      reportWindow->reportEdit()->document()->lastBlock().position());
  ...
  cursor.insertBlock(textBlockFormat);
  cursor.insertText(tr("Crecimiento de la marea paraláctica: "),
                    boldCharFormat);
  ...
  cursor.insertBlock(textBlockFormat);
  cursor.insertText(tr("Altura promedio de la marea semidiurna: "),
                    boldCharFormat);
  cursor.insertText(m_AlturaPromedioDeLaMareaSemidiurna, normalCharFormat);

  cursor.insertBlock(textBlockFormat);
  cursor.insertText(tr("Altura promedio de la marea en sicígias: "),
                    boldCharFormat);
  cursor.insertText(m_AlturaPromedioDeLaMareaSicigias, normalCharFormat);

  cursor.insertBlock(textBlockFormat);
  cursor.insertText(tr("Altura promedio de la marea en cuadratura: "),
                    boldCharFormat);
  cursor.insertText(m_AlturaPromedioDeLaMareaCuadratura, normalCharFormat);

  cursor.insertBlock(textBlockFormat);
  cursor.insertText(tr("Altura promedio de la marea tropical: "),
                    boldCharFormat);
