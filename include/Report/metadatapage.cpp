...
MetadataPage::MetadataPage(QWidget *parent) : QWizardPage(parent) {
  ...
  m_establecimientoEdit->setPlaceholderText(
      tr("(Mar, Bahía, Cabo, Población)"));
...
QString MetadataPage::ceroPuesto() const { return m_ceroPuestoEdit->text(); }

QString MetadataPage::nivelReferencia() const {
  return m_nivelReferenciaEdit->text();
}

QString MetadataPage::latitud() const { return m_latitudEdit->text(); }

QString MetadataPage::longitud() const { return m_longitudEdit->text(); }
...
QString MetadataPage::duracion() const { return m_duracionEdit->text(); }

QString MetadataPage::usoHorario() const { return m_usoHorarioEdit->text(); }
...
QString MetadataPage::responsable() const {
  return m_nombreResponsableEdit->text() + " " +
         m_apellidosResponsableEdit->text();
}

QString MetadataPage::responsabeDpto() const { return m_dptoEdit->text(); }
...
void MetadataPage::setMetaData(const ProjectMetaData &metadata) {
  ...
  QString niveldereferencia =
      QString::number(metadata.nivelReferencia(), 'f', 3) + "m";
