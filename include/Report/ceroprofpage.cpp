...
CeroProfPage::CeroProfPage(QWidget *parent):QWizardPage(parent)
{
    ...
    m_sitCeroProfGroupBox = new QGroupBox(tr("Situación del cero de profundidad respecto a"));
    m_oscNivelCeroProfGroupBox = new QGroupBox(tr("Oscilaciones del nivel del mar respecto al cero de profundidad"));

    m_ceroProfundidadEdit = new QLineEdit;
    m_metodoCeroProfundidadEdit = new QLineEdit;
    m_metodoCeroProfundidadEdit->setPlaceholderText(tr("método de cálculo del cero de profundidad"));
    m_periodoCeroProfundidadEdit = new QLineEdit;
    m_periodoCeroProfundidadEdit->setPlaceholderText(tr("tiempo determinado para calcular el cero de profundidad"));
    ...
    layout2->addRow(m_sitCeroProfundidadCeroPuestoLabel,m_sitCeroProfundidadCeroPuestoEdit);
    layout2->addRow(m_sitCeroProfundidadNMMLabel,m_sitCeroProfundidadNMMEdit);
    layout2->addRow(m_sitCeroProfundidadChapaPrincipalLabel,m_sitCeroProfundidadChapaPrincipalEdit);
...
QString CeroProfPage::nivelMinimo() const
{
    return m_nivelMinimoEdit->text();
}

QString CeroProfPage::nivelMaximo() const
{
    return m_nivelMaximoEdit->text();
}
