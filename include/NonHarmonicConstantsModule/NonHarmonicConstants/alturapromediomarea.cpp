...
AlturaPromedioMarea::AlturaPromedioMarea(
    ...
    : m_M2(M2), m_S2(S2), m_N2(N2), m_K2(K2), m_K1(K1), m_O1(O1), m_P1(P1),
      m_Q1(Q1), m_M4(M4), m_M6(M6) {
  calculate();
}

double AlturaPromedioMarea::A() {
  double resp = 1.006 * m_M2.amplitud() +
                0.27 * qPow(m_S2.amplitud(), 2) / m_M2.amplitud() -
                m_K2.amplitud();
  return resp;
}
...
double AlturaPromedioMarea::V() {
  ...
  double num1 =
      2 * a *
      qSin(qDegreesToRadians(m_K1.phase() + m_O1.phase() - m_M2.phase()));
  double den1 =
      4 * a *
          qSin(qDegreesToRadians(m_K1.phase() + m_O1.phase() - m_M2.phase())) +
      b;
  ...
  if (relationFlag <= 7.7) {
    double num2 = 2 * a *
                  qSin(qDegreesToRadians(m_K1.phase() + m_O1.phase() -
                                         m_M2.phase() - 2 * v1));
    ...
    if (val >= -1 && val <= 1)
      v2 = qRadiansToDegrees(qAsin(val));
    else
      return v1;
...
double AlturaPromedioMarea::W() {
  ...
  if (relationFlag <= 7.7) {
    ...
    double w2;
    if (val >= -1 && val <= 1)
      w2 = qRadiansToDegrees(qAsin(val));
    else
      return w1;
...
double AlturaPromedioMarea::Mn() {
  ...
  double sumando2 =
      1.071 * qPow(m_S2.amplitud(), 2) + 0.963 * qPow(m_N2.amplitud(), 2) +
      1.077 * qPow(m_K2.amplitud(), 2) + 0.269 * qPow(m_K1.amplitud(), 2) +
      0.231 * qPow(m_O1.amplitud(), 2) + 0.266 * qPow(m_P1.amplitud(), 2) +
...
void AlturaPromedioMarea::calculate() {
  ...
  double aux1Semi = m_M2.amplitud() * (qCos(qDegreesToRadians(v)) +
                                       qCos(qDegreesToRadians(w)) - 2);
  double aux2Semi = 0.035 * (v - w) * m_M4.amplitud() *
                    qSin(qDegreesToRadians(2 * m_M2.phase() - m_M4.phase()));
  double aux3Semi = 2 * m_M6.amplitud() *
                    qCos(qDegreesToRadians(3 * m_M2.phase() - m_M6.phase()));
  ...
  double aux2Sic =
      qPow((m_K1.amplitud() + m_O1.amplitud()) / m_M2.amplitud(), 2);
  ...
  //****************Altura Promedio de la Marea
  //Tropical****************************
  double aux1Trop = 0.042 * (m_K1.amplitud() + m_O1.amplitud());
  double aux2Trop = (m_K1.amplitud() + m_O1.amplitud()) *
                    (qCos(qDegreesToRadians(v)) + qCos(qDegreesToRadians(w)));
  double aux3Trop =
      0.035 * m_M2.amplitud() * (v - w) *
      qSin(qDegreesToRadians(m_M2.phase() - m_K1.phase() - m_O1.phase()));
