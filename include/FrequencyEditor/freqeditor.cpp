...
bool FreqEditor::insertRow() {
  return m_frequencyModel->insertRow(m_frequencyModel->rowCount(QModelIndex()),
                                     QModelIndex());
}

FreqEditor::FreqEditor(const QString &filePath, QWidget *parent)
  ...
  m_frequencyTable->setItemDelegate(
      new HarmonicConstantFrequencyTableDelegate(this));
...
void FreqEditor::createButtons() {
  ...
  connect(m_removeButton, SIGNAL(clicked(bool)), m_frequencyTable,
          SLOT(removeRows()));
}
...
{ out << harmonicConstant.name() << (double)harmonicConstant.frequency() <<
harmonicConstant.description();
       }
