...
QVector<int> Filters::glitchFilter(qreal glitchValue,
                                   const QVector<TidesMeasurement> &data) {
  if (data.isEmpty())
    return QVector<int>(); // Si la data esta vacia retorna un vector vacio
  ...
  for (int i = 0; i < data.size() - 1; ++i) {
    ...
    qreal delta_x = currentMeasurement.measurementDateTime().secsTo(
        nextMeasurement.measurementDateTime()); // intervalo de tiempo en horas
    delta_x /= 3600;
...
QVector<int> Filters::blocksFilter(int flag,
                                   const QVector<TidesMeasurement> &data) {
  if (data.isEmpty())
    return QVector<int>();
  if (data.size() == 1)
...
QVector<int> Filters::valueFilter(qreal value,
                                  const QVector<TidesMeasurement> &data,
                                  Filters::ByValue criteria) {
