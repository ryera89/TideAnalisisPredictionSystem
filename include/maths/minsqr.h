...
public :
    MinSqr(valarray<double> &x, valarray<double> &y, valarray<double> &sig, QVector<double> funks(const double&));
...
private:
    ...
    valarray<double> &m_measurement;   //vector donde se encuentran los valores de las mediciones
    Matrix &m_w;               //Matriz nx1 n numero de constantes armmonicas a calcular
    ...
    Matrix (*func)(const Matrix);  //Puntero a una Funcion que devulve una matriz sen y cos
    //Matrix m_Der;             //Matriz donde se guarda los senos y los cosenos de wt
    valarray<double> m_solution;   //Vector donde se van a gurdar las soluciones
    valarray<double> m_b;          //Vector donde se va a guardar el lado derecho de las ecuaciones
