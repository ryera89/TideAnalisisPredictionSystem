...
double mean(const QVector<double> &measurements);   //Valor medio de la distribucion
...
double skewnees(const QVector<double> &measurements);  //Skewnees: degree of assymetry of the distribution
double skewnees(const valarray<double> &measurements);
double kurtosis(const QVector<double> &measurements);   //Kurtosis: relative peakedness or flatness of a distribution
