...
inline void saveScalingFactor(Matrix &a, valarray<double> &scalingReg) {
  ...
  for (size_t i = 0; i < a.dim1(); ++i) {
    ...
    for (size_t j = 0; j < a.dim1(); ++j)
      if ((temp = abs(a.column(i)[j])) > big)
        big = temp;
    if (big == 0)
      throw Error("Singular Matrix in LU decomposition.");
...
LUdcmp::LUdcmp(Matrix &a) : lu(a), n(a.dim1()), indx(n), aref(a) {
  ...
  for (k = 0; k < n; ++k) {
    ...
    for (i = k; i < n; ++i) {      // seek pivot
      temp = abs(lu.column(k)[i]); // if we implement pivot with scaling factor
                                   // temp = vv[i]*abs(lu.column(k)[i])
      if (temp > big) {
        big = temp;
...
void LUdcmp::solve(valarray<double> &vb, valarray<double> &vx) {
  ...
  if (vb.size() != n)
    throw Error("ludcmp::solve: Bad sizes.");
  vx = vb;
