...
struct Customfitsvd
{
    ...
    Customfitsvd(valarray<double> &xx, valarray<double> &yy, valarray<double> &ssig, const QVector<QDateTime> &dateTime,
    QVector<double> funks(const double &, const QDateTime&, const QDateTime& ), const double TOL=1.e-12)
    : ndat(yy.size()), x(&xx), xmd(NULL), y(yy), sig(ssig),m_dateTime(dateTime),
    funcs(funks), tol(TOL) {}

    void fit() {
        int i,j,k;
        double tmp,thresh,sum;
        if (x) ma = funcs((*x)[0],m_dateTime.at(0),m_dateTime.at(0)).size();
        ...
        for (i=0;i<ndat;i++) {
            if (x) afunc=funcs((*x)[i],m_dateTime.at(i),m_dateTime.at(0));
