...
public:
    ...
    Slice_iter<double> operator [](size_t i){return row(i);}           //C style subscripting
    CSlice_iter<double> operator [](size_t i) const {return row(i);}
...
inline Matrix& Matrix::operator +=(const Matrix &m){
    if (this->dim1() == m.dim1() && this->dim2() == m.dim2()){
        this->array()+=m.array();
        return *this;
    }else throw Error("Bad size in matrix sum.");
}
inline Matrix& Matrix::operator -=(const Matrix &m){
    if (this->dim1() == m.dim1() && this->dim2() == m.dim2()){
        this->array()-=m.array();
        return *this;
    }else throw Error("Bad size in matrix rest.");
}
//-----------------------------------------------------------------------
...
inline double mul(const valarray<double> &v1, const valarray<double> &v2){
    if (v1.size() == v2.size()){
        ...
        for (size_t i = 0; i < v1.size(); ++i)
            r+=v1[i]*v2[i];
        return r;
    }else throw Error("Bad valarray size valarray-valarray multiplication.");
}
...
inline valarray<double> mul(const Matrix &m, const CSlice_iter<double> &sI){
    if (m.dim2() == sI.s.size()){
        ...
        return r;
    }else throw Error("Bad size in Matrix-CSlice_iter multiplication");
...
inline valarray<double> mul(const Matrix &m, const valarray<double> &v){
    if (m.dim2() == v.size()){
        ...
        return r;
    }else throw Error("Bad size in Matrix-valarray multiplication.");
...
inline Matrix mul(const Matrix &m1, const Matrix &m2){
    if (m1.dim2() == m2.dim1()){
        ...
        return P;
    }else throw Error("Bad size in Matrix-Matrix multiplication.");
}

//-------------------------------------------------------------------------------

//Operators overload for multiplication------------------------------------------
...
//Operator overload for sum and rest of matrices----------------------------------
...
Matrix ampMatrix(const Matrix &m1, const Matrix &m2); //return a amp matrix with m1 and m2 elements
