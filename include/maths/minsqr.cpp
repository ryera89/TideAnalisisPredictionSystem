...
MinSqr::MinSqr(valarray<double> &measurement, Matrix &w, Matrix funk(const
Matrix)):m_measurement(measurement), m_w(w), func(funk)
{
    m_nData = measurement.size();
    m_nHarmonicConstants = w.size();
    Matrix m_t(1,m_nData);
    for (int i = 0; i < m_nData; ++i) m_t[i][0] = i; //Asignamos los valores de
t desde 0 - N-1
...
MinSqr::MinSqr(valarray<double> &x, valarray<double> &y, valarray<double> &sig,
               QVector<double> funks(const double &))
    : m_nData(x.size()), m_x(x), m_y(y), m_sig(sig), funcs(funks) {
...
void MinSqr::fitLikeTheBook() {
  ...
  for (i = 0; i < m_nData; i++) { // Loop over data to accumulate coefficients
                                  // of the normal equations.
    afunc = funcs(m_x[i]);
    ym = m_y[i];
    if (mfit < m_nParam) { // Subtract off dependences on known pieces of the
                           // fitting function.
      for (j = 0; j < m_nParam; j++)
        if (!ia[j])
          ym -= a[j] * afunc[j];
    ...
    for (j = 0, l = 0; l < m_nParam;
         l++) { // Set up matrix and r.h.s. for matrix inversion.
      if (ia[l]) {
        wt = afunc[l] * sig2i;
        for (k = 0, m = 0; m <= l; m++)
          if (ia[m])
  ...
  for (j = 0; j < mfit; j++)
    for (k = 0; k < mfit; k++)
      m_covar[j][k] = temp[j][k];
  for (i = mfit; i < m_nParam;
       i++) // Rearrange covariance matrix into the correct  order.
    for (j = 0; j < i + 1; j++)
      m_covar[i][j] = m_covar[j][i] = 0.0;
