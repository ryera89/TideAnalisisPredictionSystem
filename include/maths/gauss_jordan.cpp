...
inline void seekPivot(const Matrix &a, const valarray<int> &pIndReg, size_t &prInd, size_t &pcInd){ //funcion para buscar el pivot y guarda los indexes.
    double big = 0.0;
...
inline void reduceRows(Matrix &a, Matrix &b, size_t prInd){    //Reduce las filas excepto la del pivote
    for (size_t j = 0; j < a.dim1(); ++j){
        if (j!=prInd){
            double dum = a[j][prInd];
            a[j][prInd] = 0.0;
            for (size_t l = 0; l < a.dim2();++l) a[j][l] -= a[prInd][l]*dum;
            for (size_t l = 0; l < b.dim2();++l) b[j][l] -= b[prInd][l]*dum;
        }
    }
}

void gauss_jordan(Matrix &a, Matrix &b){  //Gauss-Jordan with full pivoting strategy
