...
CustomMinSqr::CustomMinSqr(valarray<double> &x, valarray<double> &y, valarray<double> &sig, const QVector<QDateTime> &datetimeVector, QVector<double> funks(const double &, const QDateTime &, const QDateTime &)): m_nData(x.size()), m_x(x),
    m_y(y), m_sig(sig), m_dateTimeVector(datetimeVector), funcs(funks)
  {
      m_nParam = funcs(m_x[0],m_dateTimeVector.at(0),m_dateTimeVector.at(0)).size();
      a.resize(m_nParam);
