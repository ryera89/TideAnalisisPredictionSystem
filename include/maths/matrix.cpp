...
void Matrix::ampMatrix(const valarray<double> &v) {
  ...
  if (!(v.size() % this->dim1())) { // si el tamano del array es un multiplo del
    ...
    throw Error("Bad size in ampMatrix.");
}
void Matrix::ampMatrix(const NumericalVector &nv) {
  this->ampMatrix(nv.getValarray());
}

void Matrix::ampMatrix(const Matrix &m) { this->ampMatrix(m.array()); }
...
void Matrix::swapRows(size_t x, size_t y) {
  if (x < d1 && y < d1) {
    ...
    throw Error("Bad size in Row interchange.");
}
void Matrix::swapColumns(size_t x, size_t y) {
  if (x < d2 && y < d2) {
    ...
    throw Error("Bad size in Column interchange.");
}
...
bool Matrix::isTridiagonal() const {
  ...
  for (size_t i = 0; i < n - 2; ++i) {
    for (size_t j = i; j < n - 2; ++j) {
      if ((column(i)[j + 2] != 0.0) || (column(j + 2)[i] != 0.0))
        return false;
    }
  }
  return true;
...
double &Matrix::operator()(size_t x, size_t y) { return row(x)[y]; }
double Matrix::operator()(size_t x, size_t y) const { return row(x)[y]; }
//-----------------------------------------------------------
