...
public:
    ...
    T& operator () (size_t i) {return ref(curr = i);}   //FORTRAN style subscripting
...
template<class T> inline valarray<T> operator *(double d,const Slice_iter<T> &p){
...
public:
   ...
   const T& operator [] (size_t i) {return ref(curr = i);}   //C style subscripting
   const T& operator () (size_t i)  {return ref(curr = i);}   //FORTRAN style subscripting
...
template<class T> inline valarray<T> operator *(double d,const CSlice_iter<T> &p){
    valarray<T> r(p.s.size());
    for (size_t i = 0; i < r.size(); ++i){
        r[i]=d*p.ref(i);
    }
