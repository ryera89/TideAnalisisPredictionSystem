...
void TablaHorariaDeMarea::populateTable(const TidalData &m_data) {
  for (int i = 0; i < m_data.m_measurementDates.size(); ++i) {
    ...
    item->setData(Qt::DisplayRole,
                  m_data.m_dateSums.value(m_data.m_measurementDates[i]));
    item->setText(QString::number(
        m_data.m_dateSums.value(m_data.m_measurementDates[i]), 'f', 3));
    item->setTextAlignment(Qt::AlignCenter);
    item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
    ...
    item = new QTableWidgetItem;
    item->setData(Qt::DisplayRole,
                  m_data.m_meanSeaLevel.value(m_data.m_measurementDates[i]));
    item->setText(QString::number(
        m_data.m_meanSeaLevel.value(m_data.m_measurementDates[i]), 'f', 3));
    item->setTextAlignment(Qt::AlignCenter);
    item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
    ...
    item = new QTableWidgetItem;
    item->setData(Qt::DisplayRole,
                  m_data.m_extremes.value(m_data.m_measurementDates[i]).x());
    item->setText(QString::number(
        m_data.m_extremes.value(m_data.m_measurementDates[i]).x(), 'f', 3));
    item->setTextAlignment(Qt::AlignCenter);
    item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
    ...
    item = new QTableWidgetItem;
    item->setData(Qt::DisplayRole,
                  m_data.m_extremes.value(m_data.m_measurementDates[i]).y());
    item->setText(QString::number(
        m_data.m_extremes.value(m_data.m_measurementDates[i]).y(), 'f', 3));
    item->setTextAlignment(Qt::AlignCenter);
    item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
    ...
    item = new QTableWidgetItem;
    item->setData(Qt::DisplayRole,
                  m_data.m_differences.value(m_data.m_measurementDates[i]));
    item->setText(QString::number(
        m_data.m_differences.value(m_data.m_measurementDates[i]), 'f', 3));
    item->setTextAlignment(Qt::AlignCenter);
    item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
  ...
  for (int i = 0; i < m_data.m_measurementTimes.size(); ++i) {
    QTableWidgetItem *item = new QTableWidgetItem;
    item->setText(m_data.m_measurementTimes.at(i).toString(
        "hh:mm")); // setData(Qt::DisplayRole,
                   // m_data.m_measurements[i].measurementTime());
    item->setTextAlignment(Qt::AlignCenter);
    item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
  ...
  for (int i = 0; i < m_data.m_measurementDates.size(); ++i) {
    int kprima = 0;
    for (int j = 0; j < m_data.m_measurementNumberPerDay.value(
                            m_data.m_measurementDates[i]);
         ++j) {
      QTableWidgetItem *item = new QTableWidgetItem;
      item->setData(Qt::DisplayRole,
                    m_data.m_measurements[j + count].seaLevel());
      item->setText(
          QString::number(m_data.m_measurements[j + count].seaLevel(), 'f', 3));
      item->setTextAlignment(Qt::AlignCenter);
      item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
      for (int k = kprima; k < m_data.m_measurementTimes.size();
           ++k) { // Para poner el valor del nivel en la celda correcta
        if (m_data.m_measurements.at(j + count).measurementTime() ==
            m_data.m_measurementTimes.at(k)) {
      ...
      if ((kprima - 1) < (m_data.m_measurementTimes.size() - 1) &&
          j == (m_data.m_measurementNumberPerDay.value(
                    m_data.m_measurementDates[i]) -
                1)) {
    ...
    count +=
        m_data.m_measurementNumberPerDay.value(m_data.m_measurementDates[i]);
  }
  ...
  for (int column = 1; column < columnCount(); ++column) {
    ...
    for (int row = 2; row < m_data.m_measurementDates.size() + 2; ++row) {
      bool ok;
      double number =
          this->item(row, column)->data(Qt::DisplayRole).toDouble(&ok);
      if (ok)
        ++counter;
      suma += number;
    }
