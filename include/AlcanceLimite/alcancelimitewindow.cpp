...
AlcanceLimiteWindow::AlcanceLimiteWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::AlcanceLimiteWindow) {
  ui->setupUi(this);
  ...
  connect(ui->readyAPushButton, SIGNAL(clicked(bool)), this,
          SLOT(getPostAData()));
  connect(ui->readyBPushButton, SIGNAL(clicked(bool)), this,
          SLOT(getPostBData()));

  connect(ui->distanciaComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setDistanceCalcMethod(int))); // Calculo de distancias

  connect(ui->determinarPushButton, SIGNAL(clicked(bool)), this,
          SLOT(determinarAlcance()));

  connect(ui->latitudAComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setOtherFacilitiesDisabled()));
  connect(ui->longitudAComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setOtherFacilitiesDisabled()));
  connect(ui->latitudASpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setOtherFacilitiesDisabled()));
  connect(ui->longitudASpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setOtherFacilitiesDisabled()));
  connect(ui->m2AmpASpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setOtherFacilitiesDisabled()));
  connect(ui->s2AmpASpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setOtherFacilitiesDisabled()));
  connect(ui->k1AmpASpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setOtherFacilitiesDisabled()));
  connect(ui->o1AmpASpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setOtherFacilitiesDisabled()));
  connect(ui->m2PhaseASpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setOtherFacilitiesDisabled()));
  connect(ui->s2PhaseASpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setOtherFacilitiesDisabled()));
  connect(ui->k1PhaseASpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setOtherFacilitiesDisabled()));
  connect(ui->o1PhaseASpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setOtherFacilitiesDisabled()));
  connect(ui->nmmASpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setOtherFacilitiesDisabled()));

  connect(ui->latitudBComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setParameterDisabled()));
  connect(ui->longitudBComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(setParameterDisabled()));
  connect(ui->latitudBSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setParameterDisabled()));
  connect(ui->longitudBSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setParameterDisabled()));
  connect(ui->m2AmpBSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setParameterDisabled()));
  connect(ui->s2AmpBSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setParameterDisabled()));
  connect(ui->k1AmpBSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setParameterDisabled()));
  connect(ui->o1AmpBSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setParameterDisabled()));
  connect(ui->m2PhaseBSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setParameterDisabled()));
  connect(ui->s2PhaseBSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setParameterDisabled()));
  connect(ui->k1PhaseBSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setParameterDisabled()));
  connect(ui->o1PhaseBSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setParameterDisabled()));
  connect(ui->nmmBSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setParameterDisabled()));

  connect(ui->latitudAComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(resetDistanceComboBoxIndex()));
  connect(ui->longitudAComboBox, SIGNAL(currentIndexChanged(int)), this,
          SLOT(resetDistanceComboBoxIndex()));
  connect(ui->latitudASpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(resetDistanceComboBoxIndex()));
...
AlcanceLimiteWindow::~AlcanceLimiteWindow() { delete ui; }
...
void AlcanceLimiteWindow::setOtherFacilitiesDisabled() {
  if (ui->puestoBGroupBox->isEnabled())
    ui->puestoBGroupBox->setDisabled(true);

  if (ui->otrosParametrosGroupBox->isEnabled())
    ui->otrosParametrosGroupBox->setDisabled(true);
}
...
void AlcanceLimiteWindow::determinarAlcance() {
  ...
  if (m_calc_period <= 0.5) {
    ...
    ui->alcanceAlineEdit->setText(
        QString::number(m_alcance_A_FullMoon, 'g', 3) + "km");
    ui->alcanceBLineEdit->setText(
        QString::number(m_alcance_B_FullMoon, 'g', 3) + "km");
  ...
  if (m_calc_period >= 2.0) {
    ...
    ui->alcanceAlineEdit->setText(
        QString::number(m_alcance_A_MayorMoonDec, 'g', 3) + "km");
    ui->alcanceBLineEdit->setText(
        QString::number(m_alcance_B_MayorMoonDec, 'g', 3) + "km");
  ...
  if (m_calc_period > 0.5 && m_calc_period < 2.0) {
    ...
    double alcanceA = (m_alcance_A_FullMoon >= m_alcance_A_MayorMoonDec)
                          ? m_alcance_A_MayorMoonDec
                          : m_alcance_A_FullMoon;
    double alcanceB = (m_alcance_B_FullMoon >= m_alcance_B_MayorMoonDec)
                          ? m_alcance_B_MayorMoonDec
                          : m_alcance_B_FullMoon;
...
double AlcanceLimiteWindow::computeMeanAmpPhase(const HarmonicConstant &hc_A,
                                                const HarmonicConstant &hc_B) {
  double HiA_square = pow(hc_A.amplitud(), 2);
  double HiB_square = pow(hc_B.amplitud(), 2);

  double rem =
      2.0 * hc_A.amplitud() * hc_B.amplitud() * (hc_B.phase() - hc_A.phase());
...
void AlcanceLimiteWindow::determineCalcPeriod() {
  ...
  if (m_calc_period > 0.5 && m_calc_period < 2.0) {
    ui->periodoCalculoLineEdit->setText(
        tr("Sicigias y Mayor Declinación Lunar"));
    ui->lunaLLenaDateEdit->setEnabled(true);
    ui->mayorDeclinacionDateEdit->setEnabled(true);
    return;
  }
...
double AlcanceLimiteWindow::computeTnValue(const HarmonicConstant &hc_1,
                                           const HarmonicConstant &hc_2) {
...
void AlcanceLimiteWindow::determineN_FullMoon() {
  ...
  quint64 julianDaysNumber =
      fullMoonDate.toJulianDay() - epoch_date.toJulianDay();
...
void AlcanceLimiteWindow::determineNodalFactor_FullMoon() {
  m_fM2_FullMoon = 1.00035 - 0.03733 * qCos(qDegreesToRadians(m_N_FullMoon)) +
                   0.00017 * qCos(2 * qDegreesToRadians(m_N_FullMoon)) +
                   0.00001 * qCos(3 * qDegreesToRadians(m_N_FullMoon));

  m_fS2_FullMoon = 1.0;

  m_fK1_FullMoon = 1.0060 - 0.1160 * qCos(qDegreesToRadians(m_N_FullMoon)) +
                   0.0088 * qCos(2 * qDegreesToRadians(m_N_FullMoon)) +
                   0.0006 * qCos(3 * qDegreesToRadians(m_N_FullMoon));

  m_fO1_FullMoon = 1.0089 - 0.1871 * qCos(qDegreesToRadians(m_N_FullMoon)) +
                   0.0147 * qCos(2 * qDegreesToRadians(m_N_FullMoon)) +
                   0.0014 * qCos(3 * qDegreesToRadians(m_N_FullMoon));
}

void AlcanceLimiteWindow::determineAstronomicalMagnitudes_FullMoon() {
  m_v_FullMoon = 12.94 * qSin(qDegreesToRadians(m_N_FullMoon)) -
                 1.34 * qSin(2 * qDegreesToRadians(m_N_FullMoon)) +
                 0.19 * qSin(3 * qDegreesToRadians(m_N_FullMoon));

  m_ep_FullMoon = 11.87 * qSin(qDegreesToRadians(m_N_FullMoon)) -
                  1.34 * qSin(2 * qDegreesToRadians(m_N_FullMoon)) +
                  0.19 * qSin(3 * qDegreesToRadians(m_N_FullMoon));

  m_vPrima_FullMoon = 8.86 * qSin(qDegreesToRadians(m_N_FullMoon)) -
                      0.68 * qSin(2 * qDegreesToRadians(m_N_FullMoon)) +
                      0.07 * qSin(3 * qDegreesToRadians(m_N_FullMoon));

  m_vPrima2_FullMoon = 17.74 * qSin(qDegreesToRadians(m_N_FullMoon)) -
                       0.68 * qSin(2 * qDegreesToRadians(m_N_FullMoon)) +
                       0.04 * qSin(3 * qDegreesToRadians(m_N_FullMoon));
}

void AlcanceLimiteWindow::determineVoplusU_FullMoon() {
  m_V0_plus_u_M2_FullMoon = 2 * m_h0_FullMoon - 2 * m_s0_FullMoon +
                            2 * m_ep_FullMoon - 2 * m_v_FullMoon;

  m_V0_plus_u_S2_FullMoon = 0.0;

  m_V0_plus_u_K1_FullMoon = m_h0_FullMoon + 90.0 - m_vPrima_FullMoon;

  m_V0_plus_u_O1_FullMoon = m_h0_FullMoon - 2 * m_s0_FullMoon +
                            2 * m_ep_FullMoon - m_v_FullMoon + 270.0;
...
void AlcanceLimiteWindow::determineHmaxAndHmin_AB_FullMoon() {
  ...
  m_hmin_AB_FullMoon = m_LM2_AB_FullMoon * qCos(qDegreesToRadians(XM2)) +
                       m_LS2_AB_FullMoon * qCos(qDegreesToRadians(XS2)) +
                       m_LK1_AB_FullMoon * qCos(qDegreesToRadians(XK1)) +
                       m_LO1_AB_FullMoon * qCos(qDegreesToRadians(XO1));
  ...
  while (T <= 26.0) {
    ...
    double delta_h = m_LM2_AB_FullMoon * qCos(qDegreesToRadians(XM2)) +
                     m_LS2_AB_FullMoon * qCos(qDegreesToRadians(XS2)) +
                     m_LK1_AB_FullMoon * qCos(qDegreesToRadians(XK1)) +
                     m_LO1_AB_FullMoon * qCos(qDegreesToRadians(XO1));

    if (delta_h < m_hmin_AB_FullMoon)
      m_hmin_AB_FullMoon = delta_h;
    if (delta_h > m_hmax_AB_FullMoon)
      m_hmax_AB_FullMoon = delta_h;
...
void AlcanceLimiteWindow::determineHmaxAndHmin_BA_FullMoon() {
  ...
  m_hmin_BA_FullMoon = m_LM2_BA_FullMoon * qCos(qDegreesToRadians(XM2)) +
                       m_LS2_BA_FullMoon * qCos(qDegreesToRadians(XS2)) +
                       m_LK1_BA_FullMoon * qCos(qDegreesToRadians(XK1)) +
                       m_LO1_BA_FullMoon * qCos(qDegreesToRadians(XO1));
  ...
  while (T <= 26.0) {
    ...
    double delta_h = m_LM2_BA_FullMoon * qCos(qDegreesToRadians(XM2)) +
                     m_LS2_BA_FullMoon * qCos(qDegreesToRadians(XS2)) +
                     m_LK1_BA_FullMoon * qCos(qDegreesToRadians(XK1)) +
                     m_LO1_BA_FullMoon * qCos(qDegreesToRadians(XO1));

    if (delta_h < m_hmin_BA_FullMoon)
      m_hmin_BA_FullMoon = delta_h;
    if (delta_h > m_hmax_BA_FullMoon)
      m_hmax_BA_FullMoon = delta_h;
...
void AlcanceLimiteWindow::determineN_MayorMoonDec() {
  ...
  quint64 julianDaysNumber =
      MayorMoonDecDate.toJulianDay() - epoch_date.toJulianDay();
...
void AlcanceLimiteWindow::determineNodalFactor_MayorMoonDec() {
  m_fM2_MayorMoonDec = 1.00035 -
                       0.03733 * qCos(qDegreesToRadians(m_N_MayorMoonDec)) +
                       0.00017 * qCos(2 * qDegreesToRadians(m_N_MayorMoonDec)) +
                       0.00001 * qCos(3 * qDegreesToRadians(m_N_MayorMoonDec));

  m_fS2_MayorMoonDec = 1.0;

  m_fK1_MayorMoonDec = 1.0060 -
                       0.1160 * qCos(qDegreesToRadians(m_N_MayorMoonDec)) +
                       0.0088 * qCos(2 * qDegreesToRadians(m_N_MayorMoonDec)) +
                       0.0006 * qCos(3 * qDegreesToRadians(m_N_MayorMoonDec));

  m_fO1_MayorMoonDec = 1.0089 -
                       0.1871 * qCos(qDegreesToRadians(m_N_MayorMoonDec)) +
                       0.0147 * qCos(2 * qDegreesToRadians(m_N_MayorMoonDec)) +
                       0.0014 * qCos(3 * qDegreesToRadians(m_N_MayorMoonDec));
}

void AlcanceLimiteWindow::determineAstronomicalMagnitudes_MayorMoonDec() {
  m_v_MayorMoonDec = 12.94 * qSin(qDegreesToRadians(m_N_MayorMoonDec)) -
                     1.34 * qSin(2 * qDegreesToRadians(m_N_MayorMoonDec)) +
                     0.19 * qSin(3 * qDegreesToRadians(m_N_MayorMoonDec));

  m_ep_MayorMoonDec = 11.87 * qSin(qDegreesToRadians(m_N_MayorMoonDec)) -
                      1.34 * qSin(2 * qDegreesToRadians(m_N_MayorMoonDec)) +
                      0.19 * qSin(3 * qDegreesToRadians(m_N_MayorMoonDec));

  m_vPrima_MayorMoonDec = 8.86 * qSin(qDegreesToRadians(m_N_MayorMoonDec)) -
                          0.68 * qSin(2 * qDegreesToRadians(m_N_MayorMoonDec)) +
                          0.07 * qSin(3 * qDegreesToRadians(m_N_MayorMoonDec));

  m_vPrima2_MayorMoonDec =
      17.74 * qSin(qDegreesToRadians(m_N_MayorMoonDec)) -
      0.68 * qSin(2 * qDegreesToRadians(m_N_MayorMoonDec)) +
      0.04 * qSin(3 * qDegreesToRadians(m_N_MayorMoonDec));
}

void AlcanceLimiteWindow::determineVoplusU_MayorMoonDec() {
  m_V0_plus_u_M2_MayorMoonDec = 2 * m_h0_MayorMoonDec - 2 * m_s0_MayorMoonDec +
                                2 * m_ep_MayorMoonDec - 2 * m_v_MayorMoonDec;

  m_V0_plus_u_S2_MayorMoonDec = 0.0;

  m_V0_plus_u_K1_MayorMoonDec =
      m_h0_MayorMoonDec + 90.0 - m_vPrima_MayorMoonDec;

  m_V0_plus_u_O1_MayorMoonDec = m_h0_MayorMoonDec - 2 * m_s0_MayorMoonDec +
                                2 * m_ep_MayorMoonDec - m_v_MayorMoonDec +
                                270.0;
...
void AlcanceLimiteWindow::determineHmaxAndHmin_AB_MayorMoonDec() {
  ...
  m_hmin_AB_MayorMoonDec =
      m_LM2_AB_MayorMoonDec * qCos(qDegreesToRadians(XM2)) +
      m_LS2_AB_MayorMoonDec * qCos(qDegreesToRadians(XS2)) +
      m_LK1_AB_MayorMoonDec * qCos(qDegreesToRadians(XK1)) +
      m_LO1_AB_MayorMoonDec * qCos(qDegreesToRadians(XO1));
  ...
  while (T <= 26.0) {
    ...
    double delta_h = m_LM2_AB_MayorMoonDec * qCos(qDegreesToRadians(XM2)) +
                     m_LS2_AB_MayorMoonDec * qCos(qDegreesToRadians(XS2)) +
                     m_LK1_AB_MayorMoonDec * qCos(qDegreesToRadians(XK1)) +
                     m_LO1_AB_MayorMoonDec * qCos(qDegreesToRadians(XO1));

    if (delta_h < m_hmin_AB_MayorMoonDec)
      m_hmin_AB_MayorMoonDec = delta_h;
    if (delta_h > m_hmax_AB_MayorMoonDec)
      m_hmax_AB_MayorMoonDec = delta_h;
...
void AlcanceLimiteWindow::determineHmaxAndHmin_BA_MayorMoonDec() {
  ...
  m_hmin_BA_MayorMoonDec =
      m_LM2_BA_MayorMoonDec * qCos(qDegreesToRadians(XM2)) +
      m_LS2_BA_MayorMoonDec * qCos(qDegreesToRadians(XS2)) +
      m_LK1_BA_MayorMoonDec * qCos(qDegreesToRadians(XK1)) +
      m_LO1_BA_MayorMoonDec * qCos(qDegreesToRadians(XO1));
  ...
  while (T <= 26.0) {
    ...
    double delta_h = m_LM2_BA_MayorMoonDec * qCos(qDegreesToRadians(XM2)) +
                     m_LS2_BA_MayorMoonDec * qCos(qDegreesToRadians(XS2)) +
                     m_LK1_BA_MayorMoonDec * qCos(qDegreesToRadians(XK1)) +
                     m_LO1_BA_MayorMoonDec * qCos(qDegreesToRadians(XO1));

    if (delta_h < m_hmin_BA_MayorMoonDec)
      m_hmin_BA_MayorMoonDec = delta_h;
    if (delta_h > m_hmax_BA_MayorMoonDec)
      m_hmax_BA_MayorMoonDec = delta_h;
...
void AlcanceLimiteWindow::calcularAlcance_A_MayorMoonDec() {
  ...
  if (qAbs(aux_h_extr_max) >= qAbs(aux_h_extr_min)) {
    m_alcance_A_MayorMoonDec =
        (m_prof_prec * m_distance_AB) / qAbs(aux_h_extr_max);
  } else {
    m_alcance_A_MayorMoonDec =
        (m_prof_prec * m_distance_AB) / qAbs(aux_h_extr_min);
  }
}
