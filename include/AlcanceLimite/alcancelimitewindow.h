...
private:
  ...
  double computeMeanAmpPhase(
      const HarmonicConstant &hc_A,
      const HarmonicConstant &hc_B); // calculo de los valores medios de
                                     // amplitud y fase para la componente hc
  double computeCalcPeriod();
  void determineCalcPeriod();
  double computeTnValue(const HarmonicConstant &hc_1,
