...
#include <QHBoxLayout>
#include <iostream>
MeasurementUnitEditWidget::MeasurementUnitEditWidget(QWidget *parent):QFrame(parent)
{
    m_value = 0.0;
    ...
    connect(m_comboBox,SIGNAL(currentIndexChanged(int)),this,SLOT(comboBoxIndexChanged(int)));
    connect(this,SIGNAL(value_type_Changed(int,double)),this,SLOT(setValue(int,double)));
}
double MeasurementUnitEditWidget::value() const
{
    return m_value;
}
...
void MeasurementUnitEditWidget::setValue(int index,double value) //El valor guardado siempre esta en SI de unidade m o kg
{
    switch (index) {
    case 1:
        m_value = value/10;
...
void MeasurementUnitEditWidget::setSpinAndComboBoxesValues(Units u, double value)
{
    switch (u) {
    case Units::DM:
        value*=10;
