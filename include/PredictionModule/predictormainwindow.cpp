...
PredictorMainWindow::PredictorMainWindow(const QString &hcDataBaseDir,
  ...
  headersList << "Localización"
              << "Fecha Inicio"
              << "Fecha Fin";
  m_locationHCTree->setHeaderLabels(headersList);

  connect(m_locationHCTree,
  ...
  m_tidesDataTableView->setSelectionMode(
      QAbstractItemView::ContiguousSelection);
  m_tidesDataTableView->setSelectionBehavior(QAbstractItemView::SelectRows);
  m_tidesDataTableView->setMinimumWidth(340);
  m_tidesDataTableView->setSizePolicy(QSizePolicy::Fixed,
                                      QSizePolicy::Preferred);
  connect(m_tidesDataTableView, SIGNAL(rowsSelected(int, int)), this,
          SLOT(selectDataInChart(int, int)));
  connect(m_tidesDataTableView, SIGNAL(noRowsSelected()), this,
          SLOT(deselectDataInChart()));
  ...
  m_tidesHighLowWaterTableView->setSelectionMode(
      QAbstractItemView::ContiguousSelection);
  m_tidesHighLowWaterTableView->setSelectionBehavior(
      QAbstractItemView::SelectRows);
  m_tidesHighLowWaterTableView->setMinimumWidth(340);
  m_tidesHighLowWaterTableView->setSizePolicy(QSizePolicy::Fixed,
                                              QSizePolicy::Preferred);

  m_tidesDataModel = new ReadOnlyTableModel;
  connect(m_tidesDataModel, &ReadOnlyTableModel::modelReset, this,
          &PredictorMainWindow::setChartSeriesData);
  ...
  connect(m_tidesDataChartView,
          &customChartView::seriesPointsPressedAndRealesed, this,
          &PredictorMainWindow::selectPoints);
  connect(m_tidesDataChartView, &customChartView::seriesPointPressed, this,
          &PredictorMainWindow::selectPoint);
  //---------------------------------------------------------------
  ...
  connect(m_zoomXSlider, &QAbstractSlider::valueChanged, m_zoomXSpinBox,
          &QSpinBox::setValue);
  connect(m_zoomXSpinBox, SIGNAL(valueChanged(int)), m_zoomXSlider,
          SLOT(setValue(int)));
  connect(m_zoomXSpinBox, SIGNAL(valueChanged(int)), this,
          SLOT(zoomXAxis(int)));
...
void PredictorMainWindow::setHarmonicConstants(
...
void PredictorMainWindow::scanHCDataBase(const QString &hcDataBaseDir) {
  ...
  if (!hcDir.exists()) {
    QMessageBox::information(
        this, tr("Directorio no encontrado"),
        tr("El directorio %1 no existe").arg(hcDataBaseDir));
    ...
    if (provinciaList.isEmpty())
      return; // Si no hay archivos de HC no se hace nada.
...
void PredictorMainWindow::createPredictionSettingsDialog() {
  m_predictionSettingsWidget =
      new PredictonSettingsWidget(m_predictionSettings);

  connect(m_predictionSettingsWidget, &PredictonSettingsWidget::okButtonPressed,
          this, &PredictorMainWindow::getPredictionSettings);
...
void PredictorMainWindow::getPredictionSettings() {
  ...
  m_predictionSettings.setBegingDate(
      m_predictionSettingsWidget->fromDateTime());
  ...
  m_predictionSettings.setMethod(
      m_predictionSettingsWidget->predictionMethod());
...
void PredictorMainWindow::beginPredictionAtRegularIntervals() {
  ...
  // future =
  // QtConcurrent::run(predictionWithNodalFactorCte,nmm,from,to,interval,m_harmonicConstantsSet);

  // future =
  // QtConcurrent::run(predictionWithNodalFactorVar,nmm,from,to,interval,m_harmonicConstantsSet);

  switch (m_predictionSettings.method()) {
  case PredictionSettings::NF_CTE:
    future = QtConcurrent::run(predictionsWithNodalFactorCte, nmm, from, to,
                               interval, m_harmonicConstantsSet);
    break;
  default:
    future = QtConcurrent::run(predictionsWithNodalFactorVar, nmm, from, to,
                               interval, m_harmonicConstantsSet);
    break;
  }
...
void PredictorMainWindow::beginFindHighLowWaters() {
  ...
  switch (m_predictionSettings.method()) {
  case PredictionSettings::NF_CTE:
    future = QtConcurrent::run(findExtremesWithNodalFactorsCte, nmm, from, to,
                               m_harmonicConstantsSet);
    break;
  default:
    future = QtConcurrent::run(findExtremesWithNodalFactorsVar, nmm, from, to,
                               m_harmonicConstantsSet);
    break;
  }
...
void PredictorMainWindow::beginDetermineNMT() {
  ...
  m_NMTLineEdit->setText(QString::number(future.result().seaLevel(), 'f', 3) +
                         "m");
}

void PredictorMainWindow::savePredicitionsToFile() {
  QString filePath =
      QFileDialog::getSaveFileName(this, tr("Guardar Datos"), "", "*.txt");
...
void PredictorMainWindow::setChartSeriesData() {
  ...
  if (!m_tidesDataModel->measurementData().isEmpty()) {
    datetime0 =
        m_tidesDataModel->measurementData().first().measurementDateTime();
    datetimef =
        m_tidesDataModel->measurementData().last().measurementDateTime();
  }
...
void PredictorMainWindow::setAnimationOptions(
    QChart::AnimationOptions animation) {
...
void PredictorMainWindow::selectPoints(QPointF pPoint, QPointF rPoint) {
  QPointF pSeriesPoint =
      m_tidesDataChart->mapToValue(pPoint, m_tidesDataSeries);
  QPointF rSeriesPoint =
      m_tidesDataChart->mapToValue(rPoint, m_tidesDataSeries);
  ...
  if (!rowIndexes.isEmpty()) {
    m_tidesDataTableView->setRowSelection(0, rowIndexes.first(), 2,
                                          rowIndexes.last(),
...
void PredictorMainWindow::selectPoint(QPointF point) {
  ...
  foreach (QPointF p, m_tidesDataSeries->pointsVector()) {
    seriesPointsVector.append(
        m_tidesDataChart->mapToPosition(p, m_tidesDataSeries));
  }
  ...
  if (distance <= 5) {
    QPointF selectedPoint =
        m_tidesDataChart->mapToValue(closest, m_tidesDataSeries);
...
void PredictorMainWindow::zoomXAxis(int zoomLevel) {
  if (m_tidesDataSeries) {
    if (m_tidesDataSeries->pointsVector().isEmpty())
      return;
...
void PredictorMainWindow::deselectDataInChart() { m_selectionSeries->clear(); }

void PredictorMainWindow::loadHCFromDataBase(QTreeWidgetItem *current,
                                             QTreeWidgetItem *prev) {
  Q_UNUSED(prev);

  if (m_hashForHCFiles.contains(current) &&
...
void PredictorMainWindow::createActions() {
  m_predictionSettingsAction =
      new QAction(QIcon(":images/configure2.png"), tr("Configuración"));
  connect(m_predictionSettingsAction, &QAction::triggered, this,
          &PredictorMainWindow::createPredictionSettingsDialog);

  m_predictionRegularIntervalAction =
      new QAction(QIcon(":images/arrow_right.png"), tr("Intervalos Regulares"));
  m_predictionRegularIntervalAction->setEnabled(false);
  connect(m_predictionRegularIntervalAction, &QAction::triggered, this,
          &PredictorMainWindow::beginPredictionAtRegularIntervals);

  m_predictionHighLowWatersAction =
      new QAction(QIcon(":images/arrow_right.png"), tr("Máximos y Mínimos"));
  m_predictionHighLowWatersAction->setEnabled(false);
  connect(m_predictionHighLowWatersAction, &QAction::triggered, this,
          &PredictorMainWindow::beginFindHighLowWaters);

  m_detNMTAction =
      new QAction(QIcon(":images/arrow_right.png"), tr("Nivel Mínimo Teórico"));
  m_detNMTAction->setEnabled(false);
  connect(m_detNMTAction, &QAction::triggered, this,
          &PredictorMainWindow::beginDetermineNMT);

  m_tidesTableAction =
      new QAction(QIcon(":images/timetable.png"), tr("Tabla de Marea"));
  connect(m_tidesTableAction, &QAction::triggered, this,
          &PredictorMainWindow::createTidesTable);

  m_savePredictionsToFile =
      new QAction(QIcon(":images/writeToFile1.png"), tr("Guardar Prediciones"));
  connect(m_savePredictionsToFile, &QAction::triggered, this,
          &PredictorMainWindow::savePredicitionsToFile);

  m_saveHighAndLowWatersToFile = new QAction(QIcon(":images/writeToFile1.png"),
                                             tr("Guardar Niveles Extremos"));
  connect(m_saveHighAndLowWatersToFile, &QAction::triggered, this,
          &PredictorMainWindow::saveHighLowWatersToFile);
}
...
bool PredictorMainWindow::loadHCFromDataBaseFile(const QString &filePath) {
  ...
  m_harmonicConstantsSet
      .clear(); // Limpiando el set anterior para cargar el set nuevo
  ...
  if (file.open(QIODevice::ReadOnly)) {
    ...
    if (hcmagic !=
        SPMmainWindow::HCMagicNumber) { // Chequeo al tipo de archivo abierto
      QMessageBox::information(
          this, tr("Error Leyendo Archivo"),
          tr("El archivo %1 no es del tipo de .hcr").arg(file.fileName()));
      file.close();
      return false;
    }
    ...
    for (quint32 i = 0; i < dataNumber; ++i) {
      ...
      case 0: // SOLAR componentes de origen solar f = 1.0 u = 0.0
        ...
        break;
      case 1: // LUNAR_LONGPERIOD_1  componentes Mm, Msf u = 0, f =
              // [2/3-pow(sin(I),2)]/0.5021
        hcOrigin = HarmonicConstant::LUNAR_LONGPERIOD_1;
        break;
      case 2: // LUNAR_LONGPERIOD_2  componente Mf u = -2ξ, f =
              // pow(sin(I),2)/0.1578
        hcOrigin = HarmonicConstant::LUNAR_LONGPERIOD_2;
        break;
      case 3: // LUNAR_DIURNAL_1 componentes lunares diurnas
              // O1,Q1,2Q1,RHO1,SIGMA1,SO3 u = 2ξ-ν, f =
              // sin(I)*pow(cos(I/2),2)/0.3800
        hcOrigin = HarmonicConstant::LUNAR_DIURNAL_1;
        break;
      case 4: // LUNAR_DIURNAL_2 componentes lunares diurnas
              // J1,CHI1,THETA1,MP1,SO1 u = -ν, f = sin(2I)/0.7214
        hcOrigin = HarmonicConstant::LUNAR_DIURNAL_2;
        break;
      case 5: // LUNAR_DIURNAL_3 lunares diurnas OO1, KQ1 f =
              // sin(I)*pow(sin(I/2),2)/0.0164, u = -2ξ-ν
        hcOrigin = HarmonicConstant::LUNAR_DIURNAL_3;
        break;
      case 6: // COMBINATION_DIURNAL_1 componente M1 f =
              // sin(I)*pow(cos(I/2),2)/(0.3800*Qa) = f(O1)/Qa, u = ξ-ν+Q
        hcOrigin = HarmonicConstant::COMBINATION_DIURNAL_1;
        break;
      case 7: // COMBINATION_DIURNAL_2 componentes K1, SK3, f =
              // sqrt(0.8965*pow(sin(2I),2)+0.6001*sin(2I)cos(ν) + 0.1006),u = -
              // ν’
        hcOrigin = HarmonicConstant::COMBINATION_DIURNAL_2;
        break;
      case 8: // LUNAR_SEMIDIURNAL_1 componentes lunares semidiurnas
              // M2,N2,2N2,NU2,LAMDA2,MU2,2SM2,MS4,2SM6
  ...
  QMessageBox::information(
      this, tr("Error Leyendo Archivo"),
      tr("El archivo %1 no se pudo leer").arg(file.fileName()));
  return false;
}
...
void PredictorMainWindow::readHCInfoFromFile(const QString &infoFilePath) {
  ...
  if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    // return tr("No existe ningún registro de información sobre este set de
    // constantes armónicas.").toUpper();
    m_hcDescriptionPlainTextEdit->append(
        tr("No existe ningún registro de información sobre este set de "
...
void PredictorMainWindow::displayHCDescriptions() {
  ...
  cur.insertBlock();
  cur.insertText("Longitud: " + QString::number(m_longitud, 'f', 3),
                 textFormat);
