...
void MyTableView::setRowSelection(const int &column0, const int &row0, const int &columnf, const int &rowf, QItemSelectionModel::SelectionFlag flag)
{
...
void MyTableView::keyPressEvent(QKeyEvent *event)
{
    ...
    if (!indexList.isEmpty()){
        if (m_modelIndexesList.isEmpty()){
            ...
            if (m_modelIndexesList.first() != indexList.last() || m_modelIndexesList.last() != indexList.last()){
                int row0 = indexList.first().row();
                int rowf = indexList.last().row();
    ...
    if (event->key() == Qt::Key_Delete) emit deleteKeyPressed();
}
