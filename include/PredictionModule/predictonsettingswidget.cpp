...
PredictonSettingsWidget::PredictonSettingsWidget(const PredictionSettings &predSetting,QWidget *parent) : QWidget(parent)
{
    ...
    m_intervalSpinBox->setValue(predSetting.interval()/60);  //Llevando de segundos a minnutos

    m_okPushButton = new QPushButton("Aceptar");
    connect(m_okPushButton,&QAbstractButton::clicked,this,&PredictonSettingsWidget::okButtonPressed);
...
PredictionSettings::PredMethod PredictonSettingsWidget::predictionMethod() const
{
...
double PredictonSettingsWidget::nmm() const
{
    return m_nmmSpinBox->value();
}
...
{
    return m_intervalSpinBox->value()*60; //El intervalo es en segundos y el usuario lo entra en minutos
}
