...
AverageDialog::AverageDialog(const QVector<TidesMeasurement> &data,
                             QWidget *parent)
    : QDialog(parent), ui(new Ui::AverageDialog), m_data(data) {
  ui->setupUi(this);
  ...
  ui->statusLabel->setText(
      tr("Puntos de Entrada <b><font color = red>%1</b></font>. ")
          .arg(m_data.size()));
...
AverageDialog::~AverageDialog() { delete ui; }
...
void AverageDialog::averageData() {
  ...
  QDateTime controlDateTime =
      m_data.first().measurementDateTime().addSecs(timeWindowInSec);
  ...
  foreach (TidesMeasurement measurement, m_data) {
    if (measurement.measurementDateTime() <= controlDateTime) {
      ...
      m_averagedData.push_back(TidesMeasurement(
          level_ave, QDateTime::fromMSecsSinceEpoch(time_ave)));
      ++aux;
...
void AverageDialog::updateStatusLabel(int dataNumber) {
  ui->statusLabel->setText(
      tr("Puntos de Entrada <b><font color = red>%1</b></font>. ")
          .arg(m_data.size()) +
      tr("Puntos de Salida <b><font color = green>%1</b></font>.")
          .arg(dataNumber));
}
