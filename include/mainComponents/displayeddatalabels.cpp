...
DisplayedDataLabels::DisplayedDataLabels(const QString &label,
                                         const QDate &date, const QTime &time,
                                         const double &value, QWidget *parent)
    : QLabel(parent), m_label(label), m_date(date), m_time(time),
      m_value(value) {
  updateDisplayedData();
}
...
DisplayedDataLabels::DisplayedDataLabels(const QString &label,
                                         ...
                                         const double &value, QWidget *parent)
    : QLabel(parent), m_label(label), m_date(date_time.date()),
      m_time(date_time.time()), m_value(value) {
  updateDisplayedData();
}

void DisplayedDataLabels::setInternalData(const QString &label,
                                          const QDate &date, const QTime &time,
                                          const double &value) {
  m_label = label;
  m_date = date;
...
void DisplayedDataLabels::setInternalData(const QDate &date, const QTime &time,
                                          const double &value) {
  m_date = date;
  m_time = time;
  m_value = value;
...
void DisplayedDataLabels::updateDisplayedData() {
  if (m_date.isValid() && m_time.isValid()) {
    this->setText(m_label + ": " + m_date.toString("dd/MM/yyyy") + " " +
                  m_time.toString("hh:mm") + " " +
                  QString::number(m_value, 'g', 4) + "m");
  } else {
