...
QVector<HarmonicConstant> SPMmainWindow::m_selectedHarmonicConstantVector =
...
SPMmainWindow::SPMmainWindow(QWidget *parent) : QMainWindow(parent) {
  ...
  connect(m_central->tableModel(), SIGNAL(modelReset()), this,
          SLOT(hidePresentation()));
...
QVector<double> SPMmainWindow::funcion(const double &t) {
  ...
  foreach (HarmonicConstant harmonic,
...
QVector<double> SPMmainWindow::funcionModificada(const double &t,
...
QSize SPMmainWindow::sizeHint() const { return this->maximumSize(); }
...
void SPMmainWindow::getNonHarmonicConstansForReport() {
  ...
  m_CrecimientoDeLaMareaSemidiurna =
      m_nonHarmonicConstantDialog->crecimientoDeLaMareaSemidiurna();
  m_CrecimientoDeLaMareaParalactica =
      m_nonHarmonicConstantDialog->crecimientoDeLaMareaParalactica();
  m_CrecimientoDeLaMareaDiurna =
      m_nonHarmonicConstantDialog->crecimientoDeLaMareaDiurna();
  m_HoraCotidianaDeLaMareaSemidiurna =
      m_nonHarmonicConstantDialog->horaCotidianaMareaSemidiurna();
  m_HoraCotidianaDeLaMareaDiurna =
      m_nonHarmonicConstantDialog->horaCotidianaMareaDiurna();
  m_AlturaPromedioDeLaMareaSemidiurna =
      m_nonHarmonicConstantDialog->alturaPromedioDeLaMareaSemidiurna();
  m_AlturaPromedioDeLaMareaSicigias =
      m_nonHarmonicConstantDialog->alturaPromedioDeLaMareaSicigias();
  m_AlturaPromedioDeLaMareaCuadratura =
      m_nonHarmonicConstantDialog->alturaPromedioDeLaMareaCuadratura();
  m_AlturaPromedioDeLaMareaTropical =
      m_nonHarmonicConstantDialog->alturaPromedioDeLaMareaTropical();
  m_RelacionDeAmplitud = m_nonHarmonicConstantDialog->relacionDeAmplitud();
}

void SPMmainWindow::about() {
  QMessageBox::about(
      this, tr("Acerca de SIPMAR"),
      tr("<h2>SIPMAR v2.0</h2>"
         "<p><h3>2017 Equipo de Desarrollo UDI Geocuba Estudios "
         "Marinos.<h3></p>"
         "<p>El sistema de procesamiento de mareas es un software de "
         "escritorio creado para el"
         " procesamiento y análisis de los datos de marea que son registrados "
         "por las estaciones"
         " mareográficas, con el objetivo de garantizar las correcciones "
         "necesaria para los"
         " levantamientos hidrográficos y geofísicos.</p>"
         "<p>El SPM cuenta con funcionalidades que permiten la edición y "
         "análisis de los datos de"
         "marea según las nuevas tendencias internacionales asi como la "
         "corrección de las componentes"
         ...
         "<p>Consulte el Manual de Usuario para esclarecer dudas o para "
         "conocer los paso exactos"
         " que necesita para procesar su información.</p>"));
}

void SPMmainWindow::newProject() {
  if (!m_firstOpen) {
    int resp = QMessageBox::question(this, tr("Nuevo Proyecto"),
                                     tr("Se eliminarán los datos del "
                                        "proyecto. \n¿Desea continuar?"));
    if (resp == QMessageBox::Yes) {
      ...
      connect(m_projectMetaDataDialog, SIGNAL(okButtonClicked(bool)), this,
              SLOT(updateMetaData()));
    }
  } else {
    m_metadataStorage = ProjectMetaData();
    ...
    m_projectMetaDataDialog =
        new MetaDataDialog(m_metadataStorage, this, Qt::WindowCloseButtonHint);
    m_projectMetaDataDialog->show();
    connect(m_projectMetaDataDialog, SIGNAL(okButtonClicked(bool)), this,
            SLOT(updateMetaData()));
  }
...
void SPMmainWindow::openProject() {
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Abrir Proyecto"), "", tr("Archivos thd (*.thd)"));
...
void SPMmainWindow::saveProject() {
  if (m_projectFilePath.isEmpty()) {
    QString fileName = QFileDialog::getSaveFileName(
        this, tr("Guardar Proyecto"), "", tr("Archivos thd (*.thd)"));
...
void SPMmainWindow::createManualDataIntWidget() {
  ...
  connect(m_manualDataIntroductionWidget, SIGNAL(okButtonClicked()), this,
          SLOT(beginDataExtration()));
  connect(m_manualDataIntroductionWidget, SIGNAL(appendDataTrigered()), this,
          SLOT(appendManualDataToProject()));
...
void SPMmainWindow::crearTablaHoraria() {
  if (m_central->tableModel()->measurementData().isEmpty()) {
    QMessageBox::information(this, tr("Error"),
                             tr("No existen datos validos que mostrar"));
  } else {
...
void SPMmainWindow::createNonHarmonicDialog() {
  m_nonHarmonicConstantDialog =
      new NonHarmonicCalcDialog(m_metadataStorage.longitud(), this);
  m_nonHarmonicConstantDialog->loadHarmonicConstants(
      SPMmainWindow::m_selectedHarmonicConstantVector);
  m_nonHarmonicConstantDialog->setMetaData(m_metadataStorage);
  connect(m_nonHarmonicConstantDialog, SIGNAL(closed()), this,
          SLOT(getNonHarmonicConstansForReport()));
  m_nonHarmonicConstantDialog->show();
}

void SPMmainWindow::createNivelacionAcuaticaWidget() {
  m_nivelacionAcuaticaWidget =
      new NivelacionAcuaticaWidget(m_central->tableModel()->measurementData());
...
void SPMmainWindow::updateMetaData() {
  ...
  m_metadataStorage.setLocalizationName(
      m_projectMetaDataDialog->localizationName());
  m_metadataStorage.setCeroPuesto(m_projectMetaDataDialog->ceroPuesto());
  m_metadataStorage.setNivelReferencia(
      m_projectMetaDataDialog->nivelReferencia());
  m_metadataStorage.setCeroUnits(m_projectMetaDataDialog->ceroPuestoUnit());
  m_metadataStorage.setReferenceUnits(
      m_projectMetaDataDialog->nivelReferenciaUnit());
  ...
  m_metadataStorage.setTimeZoneOffSet(
      m_projectMetaDataDialog->timeZoneOffset());
  m_metadataStorage.setDayLightTimeSaving(
      m_projectMetaDataDialog->isDaylightTimeSaving());
...
void SPMmainWindow::createHarmonicAnalisisDialog() {
  if (!createHarmonicAnalisisDialogFromConfigFile()) {
    ...
    foreach (QString str, componets) {
      if (str == "M2" || str == "S2" || str == "N2" || str == "K2" ||
          str == "K1" || str == "O1" || str == "P1" || str == "Q1" ||
          str == "M4" || str == "M6") {
    ...
    if (!m_central->tableModel()->measurementData().isEmpty()){
        iniDateTime =
    QDateTime(m_central->tableModel()->measurementData().first().measurementDate(),m_central->tableModel()->measurementData().first().measurementTime(),Qt::UTC);
        endDateTime =
    QDateTime(m_central->tableModel()->measurementData().last().measurementDate(),m_central->tableModel()->measurementData().last().measurementTime(),Qt::UTC);
    }*/

    m_schemeWidget = new SchemeWidget(schemes, componets, schemesComponents,
                                      descriptionMap, this);
  }
  connect(m_schemeWidget, SIGNAL(analizeButtonClicked()), this,
          SLOT(harmonicAnalisis()));
  // connect(m_schemeWidget,SIGNAL(saveDataButtonClicked()),this,SLOT(saveAnalisisData()));
  connect(m_schemeWidget, SIGNAL(saveHarmonicConstantsButtonClicked()), this,
          SLOT(saveHarmonicConstantToFile()));
  connect(this, SIGNAL(harmonicAnalisisFinished()), m_schemeWidget,
          SLOT(enableSaveHarmonicConstantButton()));
  connect(m_schemeWidget, SIGNAL(uploadHCButtonClicked()), this,
          SLOT(createUpHcDialog()));
  m_schemeWidget->show();
}
...
void SPMmainWindow::createSamplingDilalog() {
  if (m_central->tableModel()->measurementData().isEmpty()) {
    QMessageBox::information(this, tr("Error"),
                             tr("No hay datos para realizar sampling"));
    return;
  }
  m_samplingDialog =
      new SamplingDialog(m_central->tableModel()->measurementData(), this);
...
void SPMmainWindow::createFilterDialog() {
  m_filterDialog =
      new FiltersDialog(m_central->tableModel()->measurementData(), this);
...
void SPMmainWindow::createAverageDialog() {
  m_averageDialog =
      new AverageDialog(m_central->tableModel()->measurementData(), this);
...
void SPMmainWindow::createPredictor() {
  ...
  m_predictor->setAnimationOptions(
      m_central->chartView()->chart()->animationOptions());
...
void SPMmainWindow::createReportWizard() {
  ...
  m_reportWizard->m_CrecimientoDeLaMareaSemidiurna =
      m_CrecimientoDeLaMareaSemidiurna;
  m_reportWizard->m_CrecimientoDeLaMareaParalactica =
      m_CrecimientoDeLaMareaParalactica;
  m_reportWizard->m_CrecimientoDeLaMareaDiurna = m_CrecimientoDeLaMareaDiurna;
  m_reportWizard->m_HoraCotidianaDeLaMareaSemidiurna =
      m_HoraCotidianaDeLaMareaSemidiurna;
  m_reportWizard->m_HoraCotidianaDeLaMareaDiurna =
      m_HoraCotidianaDeLaMareaDiurna;
  m_reportWizard->m_AlturaPromedioDeLaMareaSemidiurna =
      m_AlturaPromedioDeLaMareaSemidiurna;
  m_reportWizard->m_AlturaPromedioDeLaMareaSicigias =
      m_AlturaPromedioDeLaMareaSicigias;
  m_reportWizard->m_AlturaPromedioDeLaMareaCuadratura =
      m_AlturaPromedioDeLaMareaCuadratura;
  m_reportWizard->m_AlturaPromedioDeLaMareaTropical =
      m_AlturaPromedioDeLaMareaTropical;
  m_reportWizard->m_RelacionDeAmplitud = m_RelacionDeAmplitud;

  m_reportWizard->setMetaData(m_metadataStorage);
  if (!m_central->tableModel()->measurementData().isEmpty()) {
    m_reportWizard->setMeasurementsPeriod(
        m_central->tableModel()->measurementData().first().measurementDate(),
...
void SPMmainWindow::createManualHCDialog() {
  ...
  m_manualHCIntroDialog->setWindowTitle(
      tr("Introducción manual de constantes armónicas"));
...
/*bool SPMmainWindow::saveFrequencyFile()
{
    if (this->writeFrequencyFile(m_frequencyFilePath)){
        m_harmonicConstantVector =
m_frequencyEditor->harmonicConstantsModel()->allData();
...
void SPMmainWindow::harmonicAnalisisWithAllData() {
  ...
  QMap<QString, bool> harmonicComponentStatus =
      m_schemeWidget->selectedSchemeComponentStatus();
  ...
  if (m_central->tableModel()->measurementData().size() <
      SPMmainWindow::m_selectedHarmonicConstantVector.size()) {
    QMessageBox::information(this, tr("Error de Datos"),
                             tr("Numero de mediciones insuficientes "
  ...
  // Para Variante 1
  // int year =
  // m_central->tableModel()->measurementData().first().measurementDate().year();
  // QDateTime yearBegin(QDate(year,1,1),QTime(0,0),Qt::UTC);

  valarray<double> timeValarray(
      m_central->tableModel()->measurementData().size());
  valarray<double> levelValarray(
      m_central->tableModel()->measurementData().size());

  int i = 0;
  QVector<QDateTime>
      dateTimeVector; // Date Vector para la forma de analisis modificado
  QDateTime firstDateTime(
      m_central->tableModel()->measurementData().first().measurementDateTime());
  foreach (TidesMeasurement meas, m_central->tableModel()->measurementData()) {
    ...
    qreal timeInHours =
        seconds /
        3600.0; // + 1; //+1 porque las mediciones empiezan a partir de 1;
  ...
  if (m_schemeWidget->harmonicAnalisisTypeMethod() == 0) {
    if (m_schemeWidget->equationSystemSolutionMethod() == 0) {
      MinSqr minimosCuadrados(timeValarray, levelValarray, var,
                              SPMmainWindow::funcion);
      minimosCuadrados.fitLU();

      for (int k = 0;
           k < SPMmainWindow::m_selectedHarmonicConstantVector.size(); ++k) {
        SPMmainWindow::m_selectedHarmonicConstantVector[k].setComponentValues(
            minimosCuadrados.parametros()[k + 1],
            minimosCuadrados.parametros()
                [k + 1 +
                 (SPMmainWindow::m_selectedHarmonicConstantVector.size())]);
      }
    ...
    if (m_schemeWidget->equationSystemSolutionMethod() == 0) {
      CustomMinSqr minimosCuadrados(timeValarray, levelValarray, var,
                                    dateTimeVector,
...
/*void SPMmainWindow::harmonicAnalisisWithCustomData()
{
    ...
    if (initialDateTime >= endDateTime){
        QMessageBox::information(this,tr("Error en analisis"),tr("La fecha final
es menor que la fecha inicial.")); return;
    }
...
m_schemeWidget->selectedSchemeComponentStatus();
    ...
    //Para Variante 1
    //QDateTime
yearBegin(QDate(initialDateTime.date().year(),1,1),QTime(0,0),Qt::UTC);
    ...
    QDateTime firstDateTime;
    bool flag = true; //Para guardar la primera medicion que se va a usar en el
calculo foreach (TidesMeasurement meas,
m_central->tableModel()->measurementData()) { if (auxDateTime <= endDateTime){
            ...
            if (auxDateTime == meas.measurementDateTime()){
                ...
                //quint64 seconds =
yearBegin.secsTo(QDateTime(meas.measurementDate(),meas.measurementTime(),Qt::UTC));
                //double t = seconds/3600.0;
                ...
                qreal timeInHours = seconds/3600.0 + 1; //+1 porque las
mediciones empiezan a partir de 1;
    ...
    if (levelVector.size() <
SPMmainWindow::m_selectedHarmonicConstantVector.size()){
        QMessageBox::information(this,tr("Error de Datos"),tr("Numero de
mediciones insuficientes " "para realizar el análisis armonico.")); return;
    }
    ...
    if (m_schemeWidget->isLuDecompositionAnalisis()){
        MinSqr
minimosCuadrados(timeValarray,levelValarray,var,SPMmainWindow::funcion);
        minimosCuadrados.fitLU();



        for (int k = 0; k <
SPMmainWindow::m_selectedHarmonicConstantVector.size(); ++k){
...
void SPMmainWindow::uploadHCToDataBase() {
  ...
  if (m_provincia.isEmpty() || m_localidad.isEmpty()) {
    QMessageBox::information(
        m_upHcDialog, tr("Error Al Subir Constantes Armónicas"),
    ...
    if (!m_central->tableModel()->measurementData().isEmpty()) {
      ...
      QString fileName(m_provincia + "_" + m_localidad + "_" +
                       from.toString("dd-MM-yyyy") + "_" +
                       to.toString("dd-MM-yyyy") + ".hcr");

      QString infoFileName(m_provincia + "_" + m_localidad + "_" +
                           from.toString("dd-MM-yyyy") + "_" +
                           to.toString("dd-MM-yyyy") + ".txt");
...
bool SPMmainWindow::writeFile(const QString &filePath) {
  ...
  if (file.open(QIODevice::WriteOnly)) {
    ...
    out << int(
        m_metadataStorage.referenciaUnit());       // MeasurementUnitEditWidget;
...
{
    QString filePath = QFileDialog::getSaveFileName(this,tr("Guardar
Datos"),"","*.txt"); QFileInfo fileInfo(filePath); if
(fileInfo.suffix().isEmpty()) filePath.append(".txt");

    return saveAnalisisDataToFile(filePath);
}*/

/*void SPMmainWindow::recieveData(const QVector<QStringList> &data, int
...
void SPMmainWindow::setChartBrownSandTheme(bool checked) {
  if (checked)
    this->m_central->chartView()->chart()->setTheme(
...
void SPMmainWindow::setChartNoAnimation(bool checked) {
  if (checked)
    this->m_central->chartView()->chart()->setAnimationOptions(
        QChart::NoAnimation);
}
...
void SPMmainWindow::beginDataExtration() {
  ...
  for (int i = 0; i < size; ++i) {
    if (m_manualDataIntroductionWidget->model()
            ->measurementData()
            .at(i)
            .isValid()) {

      datos.push_back(
          m_manualDataIntroductionWidget->model()->measurementData().at(i));
      ...
      if (m_manualDataIntroductionWidget->isCorrectionRButtonChecked()) {
        ...
        double level =
            m_manualDataIntroductionWidget->determineLevelFromCorr(corr);
        datos.last().setSeaLevel(level);
      }
    }
  }

  m_metadataStorage.setProjectName(
      m_manualDataIntroductionWidget->projectName());
  m_metadataStorage.setStationName(
      m_manualDataIntroductionWidget->stationName());
  m_metadataStorage.setLocalizationName(
      m_manualDataIntroductionWidget->localizationName());
  ...
  m_metadataStorage.setNivelReferencia(
      m_manualDataIntroductionWidget->nivelReferencia());
  m_metadataStorage.setReferenceUnits(
      m_manualDataIntroductionWidget->referenceUnit());
  ...
  m_metadataStorage.setEquipmentID(
      m_manualDataIntroductionWidget->equipmentID());

  m_metadataStorage.setTimeZoneOffSet(
      m_manualDataIntroductionWidget->timeZoneOffset());
  m_metadataStorage.setDayLightTimeSaving(
      m_manualDataIntroductionWidget->isDaylightTimeSaving());
...
void SPMmainWindow::appendManualDataToProject() {
  ...
  QVector<TidesMeasurement> currentData(
      m_central->tableModel()->measurementData());

  if (!datos.isEmpty() && !currentData.isEmpty()) {
    if (currentData.first().measurementDateTime() >
        datos.last().measurementDateTime()) {
    ...
    if (datos.first().measurementDateTime() >
        currentData.last().measurementDateTime()) {
  ...
  QMessageBox::critical(
      this, tr("Error al Agregar Datos"),
      tr("Existen incompatibilidades temporales entre los datos que desea"
...
void SPMmainWindow::appendImportedData() {
  ...
  if (!datos.isEmpty() && !newData.isEmpty()) {
    if (datos.first().measurementDateTime() >
        newData.last().measurementDateTime()) {
    ...
    if (newData.first().measurementDateTime() >
        datos.last().measurementDateTime()) {
...
/*void SPMmainWindow::gettingData(const QVector<QStringList> &data, int
dateField, int timeField, int heightField)
{
...
void SPMmainWindow::createActions() {
  m_newProjectAction =
      new QAction(QIcon(":/images/project-new.png"), tr("&Nuevo"), this);
  m_newProjectAction->setShortcut(QKeySequence::New);
  m_newProjectAction->setToolTip(tr("Crear nuevo proyecto"));
  connect(m_newProjectAction, SIGNAL(triggered(bool)), this,
          SLOT(newProject()));

  m_loadProjectAction =
      new QAction(QIcon(":/images/project-open.png"), tr("&Abrir"), this);
  m_loadProjectAction->setShortcut(QKeySequence::Open);
  m_loadProjectAction->setToolTip(tr("Cargar proyecto"));
  connect(m_loadProjectAction, &QAction::triggered, this,
          &SPMmainWindow::openProject);

  m_saveProjectAction =
      new QAction(QIcon(":/images/save.png"), tr("&Guardar"), this);
  m_saveProjectAction->setShortcut(QKeySequence::Save);
  m_saveProjectAction->setToolTip(tr("Guardar proyecto"));
  connect(m_saveProjectAction, &QAction::triggered, this,
          &SPMmainWindow::saveProject);

  m_saveAsProjectAction =
      new QAction(QIcon(":/images/saveAs.png"), tr("Guardar &Como..."), this);
  m_saveAsProjectAction->setShortcut(QKeySequence::SaveAs);
  m_saveAsProjectAction->setToolTip(tr("Guardar proyecto como..."));
  connect(m_saveAsProjectAction, &QAction::triggered, this,
          &SPMmainWindow::saveProjectAs);

  m_projectMetaDataAction =
      new QAction(QIcon(":/images/project-info.png"), tr("&Información"), this);
  m_projectMetaDataAction->setToolTip(tr("Información del Proyecto"));
  connect(m_projectMetaDataAction, SIGNAL(triggered(bool)), this,
          SLOT(createMetaDataDialog()));

  m_exitAction =
      new QAction(QIcon(":/images/project-close.png"), tr("Salir"), this);
  ...
  // Analisis
  // Actions*******************************************************************************
  m_tablaHorariadeMareaAction =
      new QAction(QIcon(":/images/timetable.png"), tr("Tabla Horaria"), this);
  m_tablaHorariadeMareaAction->setToolTip(tr("Tabla horaria de marea"));
  connect(m_tablaHorariadeMareaAction, SIGNAL(triggered(bool)), this,
          SLOT(crearTablaHoraria()));

  m_harmonicAnalisisAction = new QAction(QIcon(":images/harmonic-analisis.png"),
                                         tr("Análisis Armónico"), this);
  m_harmonicAnalisisAction->setToolTip(tr("Análisis Armónico"));
  connect(m_harmonicAnalisisAction, SIGNAL(triggered(bool)), this,
          SLOT(createHarmonicAnalisisDialog()));
  m_nonHarmonicAnalisisAction =
      new QAction(QIcon(":images/non_harmonic-analisis.png"),
                  tr("Constantes No Armónicas"), this);
  m_nonHarmonicAnalisisAction->setToolTip(tr("Constantes No Armónicas"));
  connect(m_nonHarmonicAnalisisAction, SIGNAL(triggered(bool)), this,
          SLOT(createNonHarmonicDialog()));
  ...
  m_nivelacionAcuaticaAction = new QAction(
      QIcon(":images/nivelacion_acuatica.png"), tr("Nivelación Acuática"));
  m_nivelacionAcuaticaAction->setToolTip(
      tr("Nivelación Acuática del Puesto de Nivel"));
  connect(m_nivelacionAcuaticaAction, SIGNAL(triggered(bool)), this,
          SLOT(createNivelacionAcuaticaWidget()));

  m_alcanceLimiteAction =
      new QAction(QIcon(":images/radar-128.png"), tr("Alcance Límite"), this);
  m_alcanceLimiteAction->setToolTip(tr("Alcance Límite del Puesto de Nivel"));
  connect(m_alcanceLimiteAction, &QAction::triggered, this,
          &SPMmainWindow::createAlcanceLimiteWindow);

  m_predictorAction = new QAction(QIcon(":images/prediction.png"),
                                  tr("Predicción & Nivel Mínimo Teórico"));
  m_predictorAction->setToolTip(tr("Predicción & Nivel Mínimo Teórico"));
  connect(m_predictorAction, &QAction::triggered, this,
          &SPMmainWindow::createPredictor);

  m_manualHCIntroAction =
      new QAction(tr("Introducción manual de constantes armónicas"));
  connect(m_manualHCIntroAction, SIGNAL(triggered(bool)), this,
          SLOT(createManualHCDialog()));
  // ChartActions--------------------------------------------------------------
  m_themeLightAction = new QAction(tr("Claro"), this);
  m_themeLightAction->setCheckable(true);
  connect(m_themeLightAction, SIGNAL(triggered(bool)), this,
          SLOT(setChartLightTheme(bool)));

  m_themeDarkAction = new QAction(tr("Oscuro"), this);
  m_themeDarkAction->setCheckable(true);
  connect(m_themeDarkAction, SIGNAL(triggered(bool)), this,
          SLOT(setChartDarkTheme(bool)));

  m_themeBlueCeruleanAction = new QAction(tr("Azul1"), this);
  m_themeBlueCeruleanAction->setCheckable(true);
  connect(m_themeBlueCeruleanAction, SIGNAL(triggered(bool)), this,
          SLOT(setChartBlueCeruleanTheme(bool)));

  m_themeBrownSandAction = new QAction(tr("Medio"), this);
  m_themeBrownSandAction->setCheckable(true);
  connect(m_themeBrownSandAction, SIGNAL(triggered(bool)), this,
          SLOT(setChartBrownSandTheme(bool)));

  m_themeBlueIcyAction = new QAction(tr("Azul2"), this);
  m_themeBlueIcyAction->setCheckable(true);
  connect(m_themeBlueIcyAction, SIGNAL(triggered(bool)), this,
          SLOT(setChartBlueIcyTheme(bool)));

  m_themeBlueNcsAction = new QAction(tr("Azul3"), this);
  m_themeBlueNcsAction->setCheckable(true);
  connect(m_themeBlueNcsAction, SIGNAL(triggered(bool)), this,
          SLOT(setChartBlueNscTheme(bool)));

  m_themeHighcontrastAction = new QAction(tr("Alto Contraste"), this);
  m_themeHighcontrastAction->setCheckable(true);
  connect(m_themeHighcontrastAction, SIGNAL(triggered(bool)), this,
          SLOT(setChartHighContrastTheme(bool)));

  m_themeQtAction = new QAction(tr("Qt"), this);
  m_themeQtAction->setCheckable(true);
  connect(m_themeQtAction, SIGNAL(triggered(bool)), this,
          SLOT(setChartQtTheme(bool)));
  ...
  connect(m_animationNoAnimationAction, SIGNAL(triggered(bool)), this,
          SLOT(setChartNoAnimation(bool)));

  m_animationGridAxisAnimationAction = new QAction(tr("Ejes"), this);
  m_animationGridAxisAnimationAction->setCheckable(true);
  connect(m_animationGridAxisAnimationAction, SIGNAL(triggered(bool)), this,
          SLOT(setChartGridAxisAnimation(bool)));

  m_animationSeriesAnimationAction = new QAction(tr("Series"), this);
  m_animationSeriesAnimationAction->setCheckable(true);
  connect(m_animationSeriesAnimationAction, SIGNAL(triggered(bool)), this,
          SLOT(setChartSeriesAnimation(bool)));

  m_animationAllAnimationAction = new QAction(tr("Todas"), this);
  m_animationAllAnimationAction->setCheckable(true);
  connect(m_animationAllAnimationAction, SIGNAL(triggered(bool)), this,
          SLOT(setChartAllAnimation(bool)));
  ...
  connect(m_chartRenderHintAction, SIGNAL(toggled(bool)), this,
          SLOT(setRenderedChartViewFlag(bool)));
  m_chartRenderHintAction->setChecked(true);
  // view Actions--------------------------------------------------

  // Data
  // Actions---------------------------------------------------------------------
  m_manualDataIntroductionAction = new QAction(QIcon(":/images/manualData.png"),
                                               tr("Introducción Manual"), this);
  connect(m_manualDataIntroductionAction, SIGNAL(triggered(bool)), this,
          SLOT(createManualDataIntWidget()));

  m_importFrom_ASCII_Action =
      new QAction(QIcon(":/images/import.png"), tr("Importar"), this);
  m_importFrom_ASCII_Action->setToolTip(
      tr("Importar datos desde archivo ASCII"));
  connect(m_importFrom_ASCII_Action, SIGNAL(triggered(bool)), this,
          SLOT(loadDataFile()));

  // Edit ACtion
  m_samplingDialogAction = new QAction(tr("Sampling"));
  connect(m_samplingDialogAction, SIGNAL(triggered(bool)), this,
          SLOT(createSamplingDilalog()));

  m_filterDialogAction = new QAction(tr("Filtro"));
  connect(m_filterDialogAction, SIGNAL(triggered(bool)), this,
          SLOT(createFilterDialog()));

  m_averageDialogAction = new QAction(tr("Promedio"));
  connect(m_averageDialogAction, SIGNAL(triggered(bool)), this,
          SLOT(createAverageDialog()));

  m_reportAction = new QAction(QIcon(":images/report-icon.png"),
                               tr("Formulario del puesto"));
  connect(m_reportAction, &QAction::triggered, this,
          &SPMmainWindow::createReportWizard);

  m_aboutAction =
      new QAction(QIcon(":images/About.png"), tr("Acerca de SIPMAR"));
...
void SPMmainWindow::syncData(
    const QVector<HarmonicConstant>
        &components) // Para sync si hay un cambio en los componentes que se
                     // editan en otra facilidad
{
  ...
  if (file.open(QIODevice::ReadOnly)) {
    ...
    if (magicNumber != SchemesMagicNumber) {
      QMessageBox::information(
          this, tr("Error leyendo archivo"),
          tr("Error leyendo el archivo de configuracion %1")
              .arg(file.fileName()));
      return;
    }
    uint schemeNumber;
    ...
    for (uint i = 0; i < schemeNumber; ++i) {
      ...
      for (uint j = 0; j < componentsNumber; ++j) {
        ...
        bool status;
        in >> comp >> status;
        if (componentStatus.contains(comp))
    ...
    if (file.open(QIODevice::WriteOnly)) {
      ...
      foreach (QString schemeName, schemes) {
        ...
        foreach (HarmonicConstant harmonic, components) {
          out << harmonic.name()
              << scheme_componentSatus.value(schemeName).value(harmonic.name());
        }
      }
    ...
    QMessageBox::information(this, tr("Error leyendo archivo"),
                             tr("Error leyendo el archivo de configuracion %1")
                                 .arg(file.fileName()));
    return;
  }
...
bool SPMmainWindow::createHarmonicAnalisisDialogFromConfigFile() {
  ...
  if (file.open(QIODevice::ReadOnly)) {
    ...
    for (uint i = 0; i < schemeNumber; ++i) {
      ...
      for (uint j = 0; j < componentsNumber; ++j) {
        ...
        if (!components.contains(comp))
          components.append(comp); // para que los componentes se agreguen una
                                   // sola ves, los comp de todos los esqumeas
                                   // deben ser iguales, solo cambia el status
        componentStatus[comp] = status;
      }
      scheme_componentSatus[sch] = componentStatus;
      // components.clear();
    ...
    m_schemeWidget = new SchemeWidget(
        schemes, components, scheme_componentSatus, schemeDescription, this);
    return true;
  } else {
    QMessageBox::information(this, tr("Error leyendo archivo"),
                             tr("Error leyendo el archivo de configuracion %1")
...
bool SPMmainWindow::readFile(const QString &filePath) {
  ...
  if (file.open(QIODevice::ReadOnly)) {
    ...
    if (number != MagicNumber) {
      QMessageBox::information(
          this, tr("Error al leer archivo"),
          tr("El archivo %1 no tiene el formato requerido.")
              .arg(file.fileName()));
      return false;
    }
    ...
    in >> projectName >> stationName >> locationName >> timeZoneOffset >>
        daylightTimeSaving >> nivelReferencia >> unidadNivelReferencia >>
        ceroPuesto >> unidadCeroPuesto >> latitud >> longitud >> equipmetID;
    ...
    QMessageBox::information(this, tr("Error al leer archivo"),
...
/*bool SPMmainWindow::saveAnalisisDataToFile(const QString &filePath)
{
    ...
    if (file.open(QIODevice::WriteOnly)){
        ...
        if (m_schemeWidget->currentSelectionComboBoxIndex()){
            ...
            QDateTime endDateTime = m_schemeWidget->endDateTime();
            if (initialDateTime >= endDateTime){
                QMessageBox::information(this,tr("Error en analisis"),tr("La
fecha final es menor que la fecha inicial.")); return false;
            }
...
meas.measurementTime().toString("hh:mm") << " " << meas.seaLevel() << "\n";
        ...
        QMessageBox::information(this,tr("Error Guardando Datos"),tr("El archivo
%1 no se puede escribir").arg(file.fileName())); return false;
    }
    return true;
}*/
...
void SPMmainWindow::harmonicConstantSet() {
  ...
  m_harmonicConstantVector << m_Mm << m_Msf << m_Mf << m_Ssa << m_Sa << m_O1
                           << m_Q1 << m_2Q1 << m_RHO1 << m_SIGMA1 << m_J1
                           << m_CHI1 << m_THETA1 << m_MP1 << m_SO1 << m_OO1
                           << m_KQ1 << m_P1 << m_PI1 << m_PSI1 << m_PHI1 << m_S1
                           << m_M1 << m_K1 << m_M2 << m_N2 << m_2N2 << m_NU2
                           << m_LAMDA2 << m_MU2 << m_KJ2 << m_S2 << m_T2 << m_R2
                           << m_L2 << m_K2 << m_M3 << m_S3 << m_MNS2 << m_2SM2
                           << m_MK3 << m_2MK3 << m_SK3 << m_SO3 << m_M4 << m_MS4
                           << m_MN4 << m_MK4 << m_S4 << m_M6 << m_2MS6 << m_2MN6
                           << m_2SM6 << m_MSN6 << m_S6 << m_M8 << m_3MS8
                           << m_2MS8 << m_2MSN8 << m_S8;
}
...
void SPMmainWindow::applyCorrectionsToHarmonicConstants() {
  QDateTime begin =
      m_central->tableModel()->measurementData().first().measurementDateTime();
  QDateTime end =
      m_central->tableModel()->measurementData().last().measurementDateTime();
  ...
  for (int i = 0; i < m_selectedHarmonicConstantVector.size(); ++i) {
    findHarmonicConstantCorrectedPhase(begin, end, daylightsaving,
                                       m_selectedHarmonicConstantVector[i]);
    findHarmonicConstantPhase(timeOffSet, longitud,
                              m_selectedHarmonicConstantVector[i]);
    findHarmonicConstantAmplitud(begin, end, daylightsaving,
                                 m_selectedHarmonicConstantVector[i]);
  }
}
...
void SPMmainWindow::findHarmonicConstantCorrectedPhase(QDateTime begin,
                                                       QDateTime end,
                                                       bool DaylightTimeSaving,
                                                       HarmonicConstant &hc) {
...
void SPMmainWindow::findHarmonicConstantPhase(int timeOffset, double longitud,
                                              HarmonicConstant &hc) {
...
void SPMmainWindow::findHarmonicConstantAmplitud(QDateTime begin, QDateTime end,
                                                 bool DaylightTimeSaving,
                                                 HarmonicConstant &hc) {
  if (DaylightTimeSaving) {
    begin = begin.addSecs(-3600);
