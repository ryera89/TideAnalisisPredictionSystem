...
public:
  ...
  enum ByValue {
    ...
    LESS_EQUAL,
    GREATER_EQUAL
  }; // Enum para el filtro valueFilter

  QVector<int> glitchFilter(qreal glitchValue,
                            const QVector<TidesMeasurement>
                                &data); // Retorna un vector con las posiciones
                                        // de los datos encontrados
  QVector<int>
  blocksFilter(int flag,
               const QVector<TidesMeasurement>
                   &data); // Retorna un vector con la posicion de los bloques
  QVector<int> valueFilter(qreal value, const QVector<TidesMeasurement> &data,
